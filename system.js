...
function liveOutput(updates){
  ...
  const readyArtifacts=artifacts.filter(x=>x.state==='ready'&&(x.data?.readiness==='READY'||x.data?.claimVerification?.passed===true));const candidates=artifacts.length-readyArtifacts.length;
  const outputLabel=readyArtifacts.length?`${readyArtifacts.length} verified artifact${readyArtifacts.length===1?'':'s'} ready`:candidates?`${candidates} candidate artifact${candidates===1?'':'s'} · verification pending`:external.length?`No candidate artifact yet · ${external.length} source receipt${external.length===1?'':'s'}`:'Work will appear here as it becomes real';
  return `<section class="live-output"><div class="live-output-head"><div><span class="eyebrow">LIVE OUTPUT</span><h2>${outputLabel}</h2></div><span class="signal"><i></i> LIVE</span></div>${updates.length?`<div class="signal-list">${updates.slice(0,4).map((u,i)=>`<article><span class="signal-index">${String(i+1).padStart(2,'0')}</span><div><strong>${esc(u.title)}</strong><p>${esc(compact(u.body,170))}</p></div><time>${esc(fmt(u.time))}</time></article>`).join('')}</div>`:`<div class="live-empty"><span class="live-beam"></span><div><strong>Waiting for the first persisted signal</strong><p>Only durable progress appears here. No theatre, no fake activity.</p></div></div>`}</section>`;
}
...
function work(){
  ...
  <div class="mission-kpis"><div><span>NOW</span><strong>${esc(stage[0])}</strong></div><div><span>PROOF</span><strong>${evidence?`${evidence} source receipt${evidence===1?'':'s'}`:'Awaiting evidence'}</strong></div><div><span>OUTPUT</span><strong>${readyArtifacts?`${readyArtifacts} ready`:artifacts?`${artifacts} candidate${artifacts===1?'':'s'}`:'Building V1'}</strong></div></div>
