...
let companies=[],company=null,operating=null,health=null,launchingGoal='',launchError='',directAnswer=null,sendingMessage=false,lastHydrationError='';
...
async function api(path,opt={}){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),Number(opt.timeoutMs)||20000);try{const {timeoutMs:_,...request}=opt;const r=await fetch('/api/v1'+path,{headers:{'content-type':'application/json',...(request.headers||{})},...request,signal:controller.signal,body:request.body?JSON.stringify(request.body):undefined});const x=await r.json().catch(()=>({}));if(!r.ok)throw Error(x.error||'Request failed');return x}catch(e){if(e?.name==='AbortError')throw Error('This is taking too long. Please try again.');throw e}finally{clearTimeout(timeout)}}
...
function promptBox({large=false,value=''}={}){return `<form class="prompt-box ${large?'large':''}" id="goalForm"><textarea name="goal" ${large?'autofocus':''} required placeholder="What do you need done?">${esc(value)}</textarea><div class="prompt-foot"><span>${large?'Describe the result. Company Zero handles the setup.':'Be specific or just talk normally.'}</span><button aria-label="Start">${icon('arrowUp')}</button></div></form>`}
...
function home(){
  ...
  if(launchError)return `<div class="home-empty"><div class="hero-lockup"><img class="hero-organism" src="assets/brand/company-zero-mark.png" alt=""><div><span class="eyebrow">COULDN’T START</span><h1>Your request is still here.</h1><p>${esc(launchError.message)}</p></div></div>${promptBox({large:true,value:launchError.goal})}<div class="home-proof"><span>No work was created</span><span>Safe to try again</span></div></div>`;
...
function bind(){
  ...
  const gf=$('#goalForm');if(gf)gf.onsubmit=e=>{e.preventDefault();const goal=String(new FormData(e.target).get('goal')||'').trim();if(!goal)return;launchingGoal=goal;launchError='';directAnswer=null;render();act(async()=>{try{const result=await api('/interactions',{method:'POST',body:{message:goal},timeoutMs:25000});launchingGoal='';if(result.kind==='answer'){directAnswer={question:goal,answer:result.answer};company=null;operating=null;active='';localStorage.removeItem('cz_active');page='home';location.hash='home';return}operating=result.operating;company=operating.company;active=company.id;localStorage.cz_active=active;companies=[company,...companies.filter(x=>x.id!==company.id)];page='work';location.hash=page}catch(err){launchingGoal='';launchError={goal,message:`${humanError(err)} Your request was not discarded.`};throw err}},'Ready')};
...
$('#newWork').onclick=()=>{page='home';location.hash='home';company=null;operating=null;directAnswer=null;launchError='';active='';localStorage.removeItem('cz_active');render();setTimeout(()=>$('#goalForm textarea')?.focus(),20)};
