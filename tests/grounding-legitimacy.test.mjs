...
try{
  ...
  const before=calls;const worked=await new WorkerService({workerId:'grounding-operation'}).tick();assert.equal(worked.run.state,'completed');assert.equal(calls,before+1);
