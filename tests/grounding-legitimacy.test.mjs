...
try{
  ...
  const session=await startOperatingSession({goal:'Reach 5 verified units',context:{currentMetrics:{outcome_metric_1:999}},providers:[{name:'Unmeasured provider',type:'http',baseUrl,manifest:{capabilities:[{name:'return_unmapped_numbers',endpoint:'/operate',risk:'read',operationKind:'observe',acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object',required:['metrics','recordId']}},{name:'bounded_intervention',endpoint:'/operate',risk:'read',operationKind:'change',changes:['mission_state'],acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object'}},{name:'verify_without_evidence_value',endpoint:'/operate',risk:'read',operationKind:'verify',acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object'},measurements:[{metricId:'outcome_metric_1',path:'$.missing_external_metric',phase:'after',externalRefPath:'$.recordId'}]}]}}]});
  for(let i=0;i<12&&(await get(session.session.id)).state!=='operating';i++)await new WorkerService({workerId:`grounding-stage-${i}`}).tick();
  const before=calls;const worked=await new WorkerService({workerId:'grounding-operation'}).tick();assert.equal(worked.run.state,'completed');assert.equal(calls,before+3);
  ...
  const verification=await get(worked.run.data.outcomeVerificationId);assert.equal(verification.data.outcomeAchieved,false);assert.equal(verification.data.status,'insufficient_observation');
  ...
  const gated=await startOperatingSession({goal:'Keep an arbitrary measured value below 20',providers:[{name:'Relevance provider',type:'http',baseUrl,manifest:{capabilities:[{name:'measure_value',endpoint:'/operate',risk:'read',operationKind:'observe',acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object'},measurements:[{metricId:'outcome_metric_1',path:'$.metrics.outcome_metric_1',phase:'after',externalRefPath:'$.recordId'}]},{name:'change_value',endpoint:'/operate',risk:'read',operationKind:'change',changes:['outcome_metric_1'],acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object'}}]}}]});
  ...
  const second=await startOperatingSession({goal:'Keep an arbitrary measured value below 20',providers:[{name:'Second arbitrary measurement',type:'http',baseUrl,manifest:{capabilities:[{name:'observe_unfamiliar_value',endpoint:'/operate',risk:'read',operationKind:'observe',observes:['outcome_metric_1'],acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object',required:['baseline','current','baselineRef','currentRef']},measurements:[{metricId:'outcome_metric_1',path:'$.baseline',phase:'baseline',externalRefPath:'$.baselineRef'},{metricId:'outcome_metric_1',path:'$.current',phase:'after',externalRefPath:'$.currentRef'}]},{name:'change_unfamiliar_value',endpoint:'/operate',risk:'read',operationKind:'change',changes:['outcome_metric_1'],acceptsMissionEnvelope:true,inputSchema:{type:'object'},outputSchema:{type:'object'}}]}}]});
