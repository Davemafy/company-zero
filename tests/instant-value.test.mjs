...
try{
  ...
  globalThis.fetch=async (url,opts={})=>{
    ...
    if(!String(url).includes('tensormux.test'))throw Error(`unexpected_test_url:${url}`);
    tensorCalls+=1;
    ...
    if(body.model==='glm-verifier'){
      ...
      verifierCalls+=1;const verdict=verifierCalls===1?{passed:false,summary:'one unsupported claim',claims:[{id:'c1',claim:'Reach 10 customers',file:'execution.md',status:'UNSUPPORTED',reason:'not supported or labelled',support:[]}]}:{passed:true,summary:'all material claims are truthful',claims:[{id:'c1',claim:'Proposed target: reach 10 customers',file:'launch-brief.md',status:'ASSUMPTION',reason:'explicitly labelled',support:[]}]};
    ...
    if(body.model==='glm-specialist'){
      ...
      if(/bounded claim repair/i.test(system)){const content=('# Repaired launch brief\n\nProposed target: reach 10 customers. This is explicitly an assumption to validate, not a measured result. The rest of the useful draft remains intact. ').repeat(3);return new Response(JSON.stringify({id:'repair',choices:[{message:{content:JSON.stringify({title:'Repaired launch brief',summary:'Unsupported claim relabelled as an assumption.',files:[{name:'launch-brief.md',mimeType:'text/markdown',content}]})}}]}),{status:200,headers:{'content-type':'application/json'}})}
    }
  ...
  assert.ok(tensorCalls>=3,'failed role DAG should exercise planner and roles without a legacy monolithic fallback');
  ...
  mode='empty-roles';verifierCalls=0;tensorCalls=0;const emptyRoleEvents=[];
