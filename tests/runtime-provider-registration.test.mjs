...
try{
  ...
  assert.equal(rows.filter(x=>x.kind==='capability').length,3,'API-side session should start with only the built-in Studio when Railway-only Scrapy credentials are absent');
  ...
  assert.equal(world?.data?.capabilities?.length,4,'world model should include Studio plus the newly registered runtime capability');
  const stage=rows.find(x=>x.kind==='session_artifact'&&x.data.sessionId===started.session.id&&x.data.stage==='learning_world');
  assert.ok(stage?.data?.items?.some(x=>x.type==='capability_count'&&x.label==='4 declared capabilities'),'world ledger should surface the declared capability');
