...
await transitionDeliverable(companyId,baseline.id,'executing');await transitionDeliverable(companyId,baseline.id,'executed',{evidenceIds:['trace-baseline']});await transitionDeliverable(companyId,baseline.id,'verifying');await transitionDeliverable(companyId,baseline.id,'verified',{evidenceIds:['obs-baseline']});const sameState=await transitionDeliverable(companyId,baseline.id,'verified',{evidenceIds:['obs-baseline-late'],operationId:'op-baseline-late'});assert.ok(sameState.data.evidenceIds.includes('obs-baseline-late'),'same-state transition must preserve late evidence');assert.ok(sameState.data.operationIds.includes('op-baseline-late'),'same-state transition must preserve late operation receipt');
