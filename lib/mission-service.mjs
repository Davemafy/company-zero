...
async function mutateLatest(recordId,mutate,{attempts=4}={}){
  let lastError=null;
  for(let attempt=1;attempt<=attempts;attempt++){
    const current=await get(recordId);if(!current)throw new DomainError('record_not_found',404);
    const next=structuredClone(current);mutate(next,current);
    try{return await put(next,{expectedVersion:current.version})}
    catch(error){if(error?.message!=='stale_version'||attempt===attempts)throw error;lastError=error}
  }
  throw lastError||Error('metadata_update_failed');
}
const readinessRank=value=>String(value||'').toUpperCase()==='READY'?2:String(value||'').toUpperCase()==='PARTIAL'?1:0;
export async function syncArtifactMetadata({contractId,sessionId,artifact,jobId=null,evidenceCount=0,ready=false}={}){
  const incomingReadiness=ready?'READY':'PARTIAL',artifactVersion=Number(artifact?.data?.deliverableVersion||1);
  const contract=await mutateLatest(contractId,current=>{
    if(current.kind!=='deliverable_contract')throw new DomainError('deliverable_contract_not_found',404);
    const existingReadiness=current.data?.readiness||(current.state==='ready'?'READY':current.state==='partial'?'PARTIAL':null);
    const mayAdvance=readinessRank(incomingReadiness)>=readinessRank(existingReadiness);
    if(mayAdvance){
      current.state=ready?'ready':'partial';
      Object.assign(current.data,{status:ready?'ready':'partial',readiness:incomingReadiness,latestArtifactId:artifact.id,latestVersion:Math.max(Number(current.data?.latestVersion||0),artifactVersion),completedArtifactKeys:ready?['primary']:(current.data?.completedArtifactKeys||[]),updatedAt:now()});
    }else{
      Object.assign(current.data,{latestVersion:Math.max(Number(current.data?.latestVersion||0),artifactVersion),updatedAt:now()});
    }
  });
  const session=await mutateLatest(sessionId,current=>{
    if(current.kind!=='operating_session')throw new DomainError('operating_session_not_found',404);
    const existingReadiness=current.data?.readiness||(current.data?.state==='ready'?'READY':current.data?.state==='partial'?'PARTIAL':null);
    const mayAdvance=readinessRank(incomingReadiness)>=readinessRank(existingReadiness);
    current.state='active';
    const common={currentStage:'producing_progress',lastProgressAt:now(),firstArtifactId:current.data?.firstArtifactId||artifact.id,latestJobId:jobId||current.data?.latestJobId||null,evidenceCount:Number(evidenceCount||0)};
    if(mayAdvance)Object.assign(current.data,common,{state:ready?'ready':'partial',readiness:incomingReadiness,latestArtifactId:artifact.id,...(ready?{firstValueReadyAt:current.data?.firstValueReadyAt||now()}:{firstValuePartialAt:current.data?.firstValuePartialAt||now()}),resultQuality:ready?'ready':'partial'});
    else Object.assign(current.data,common);
  });
  return {contract,session};
}
async function strongestPersistedArtifact(companyId,sessionId){
  const records=await list({companyId,limit:1200});
  return records.filter(x=>x.kind==='artifact'&&['ready','partial'].includes(x.state)&&x.data?.sessionId===sessionId)
    .sort((a,b)=>readinessRank(b.data?.readiness)-readinessRank(a.data?.readiness)||Number(b.data?.deliverableVersion||1)-Number(a.data?.deliverableVersion||1)||String(b.created_at||'').localeCompare(String(a.created_at||'')))[0]||null;
}
...
async function companySnapshot(companyId,sessionId){
  ...
  const artifacts=records.filter(x=>x.kind==='artifact'&&x.data?.sessionId===sessionId&&['ready','partial'].includes(x.state)).sort((a,b)=>Number(b.data?.deliverableVersion||1)-Number(a.data?.deliverableVersion||1));
...
export async function scheduleValueFirst(companyId,sessionId,missionId=null){
  ...
  const existing=records.filter(x=>x.kind==='artifact'&&['ready','partial'].includes(x.state)&&x.data?.sessionId===sessionId).sort((a,b)=>Number(b.data?.deliverableVersion||1)-Number(a.data?.deliverableVersion||1))[0];
...
async function persistPublicEvidence(companyId,sessionId,built){
  const results=Array.isArray(built?.publicEvidence?.results)?built.publicEvidence.results.filter(item=>item?.relevanceScore==null||Number(item.relevanceScore)>0).slice(0,10):[];
  const saved=[];
  ...
  if(saved.length)await emit(companyId,sessionId,'EVIDENCE_PERSISTED',`Saved ${saved.length} relevant public source receipt${saved.length===1?'':'s'}: ${saved.slice(0,4).map(x=>x.data.subject).join(' · ')}${saved.length>4?' · …':''}`,{count:saved.length,subjects:saved.map(x=>x.data.subject)});
  return saved;
}
...
function summarizeRunTelemetry(built,totalDurationMs=null){
  ...
  return {
    ...
    timeline:built?.runtimeTimeline||null,
    roleSummary:built?.roleTelemetrySummary||null,
    calls:calls.length,
...
async function persistArtifact({companyId,session,mission,contract,built,version,previousArtifactId=null,source}){
  ...
  const artifact=record('artifact',{
    ...
    summary:String(built?.summary||(ready?'A verified result is ready.':'Useful candidate work is preserved while verification remains incomplete.')).slice(0,500),
  ...
  },companyId,ready?'ready':'partial');
  Object.assign(artifact,await put(artifact));
  const evidence=await persistPublicEvidence(companyId,session.id,built);
  const job=record('job',{sessionId:session.id,operation:ready?'Verified result ready':'Candidate result preserved — verification pending',source:'thin-product-runtime',artifactId:artifact.id,evidenceCount:evidence.length,readiness},companyId,'completed');
  Object.assign(job,await put(job));
  const synced=await syncArtifactMetadata({contractId:contract.id,sessionId:session.id,artifact,jobId:job.id,evidenceCount:evidence.length,ready});
  Object.assign(contract,synced.contract);Object.assign(session,synced.session);
  const readyMessage=ready?`Ready: ${artifact.data.title}. ${files.length} file${files.length===1?'':'s'} saved${evidence.length?` with ${evidence.length} source receipt${evidence.length===1?'':'s'}`:''}.`:`Partial result saved: ${artifact.data.title}. ${files.length} useful candidate file${files.length===1?'':'s'} preserved; verification has not passed. ${evidence.length} source receipt${evidence.length===1?'':'s'} attached.`;
...
export async function materializeFirstValue(companyId,sessionId){
  ...
  const records=await list({companyId,limit:1200});
  const existing=records.filter(x=>x.kind==='artifact'&&['ready','partial'].includes(x.state)&&x.data?.sessionId===sessionId).sort((a,b)=>Number(b.data?.deliverableVersion||1)-Number(a.data?.deliverableVersion||1))[0];if(existing)return existing;
  ...
  const runtimeTimeline={acceptedAt:startedAt,firstEventAt:null,firstArtifactAt:null,rolesCompletedAt:null,verificationStartedAt:null,verificationCompletedAt:null,pipelineCompletedAt:null};
  ...
  const onProgress=async p=>{
    if(!p?.type)return;
    const at=Date.now();if(!runtimeTimeline.firstEventAt)runtimeTimeline.firstEventAt=at;
    if(p.type==='DRAFT_ARTIFACT_AVAILABLE'&&!runtimeTimeline.firstArtifactAt)runtimeTimeline.firstArtifactAt=at;
    if(p.type==='LATENCY_ROLES_COMPLETED')runtimeTimeline.rolesCompletedAt=Number(p.data?.rolesCompletedAt||at);
    if(p.type==='LATENCY_VERIFICATION_STARTED')runtimeTimeline.verificationStartedAt=Number(p.data?.verificationStartedAt||at);
    if(p.type==='LATENCY_VERIFICATION_COMPLETED')runtimeTimeline.verificationCompletedAt=Number(p.data?.verificationCompletedAt||at);
  ...
  try{
    ...
    runtimeTimeline.pipelineCompletedAt=Date.now();built.runtimeTimeline=runtimeTimeline;
    built.runtimeTotalMs=runtimeTimeline.pipelineCompletedAt-startedAt;
    ...
    const persisted=await strongestPersistedArtifact(companyId,sessionId).catch(()=>null);
    if(persisted){
      await emit(companyId,sessionId,'PERSISTENCE_RACE_RECOVERED',`Persistence metadata conflicted after the artifact was saved (${String(error?.message||error).slice(0,100)}). Preserving ${persisted.data?.readiness||persisted.state} artifact instead of downgrading it.`,{error:String(error?.message||error),artifactId:persisted.id,readiness:persisted.data?.readiness||null,deliverableVersion:persisted.data?.deliverableVersion||null}).catch(()=>{});
      return persisted;
    }
...
export async function reviseValueMission(companyId,sessionId,message){
  ...
  const prior=records.filter(x=>x.kind==='artifact'&&['ready','partial'].includes(x.state)&&x.data?.sessionId===sessionId).sort((a,b)=>Number(b.data?.deliverableVersion||1)-Number(a.data?.deliverableVersion||1))[0]||null;
