...
async function persistArtifact({companyId,session,mission,contract,built,version,previousArtifactId=null,source}){
  ...
  const readyMessage=built?.degraded?`Partial result saved: ${artifact.data.title}. ${evidence.length} grounded source${evidence.length===1?'':'s'} attached.`:`Ready: ${artifact.data.title}. ${files.length} file${files.length===1?'':'s'} saved${evidence.length?` with ${evidence.length} grounded source${evidence.length===1?'':'s'}`:''}.`;
  await emit(companyId,session.id,built?.degraded?'RESULT_PARTIAL':'RESULT_READY',readyMessage,{artifactId:artifact.id,version,evidenceCount:evidence.length,degraded:Boolean(built?.degraded),fileNames:files.map(f=>f.name)});
