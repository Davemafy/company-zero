...
export async function createValueMission({goal,outcome,context={},constraints={},ownerSessionId=null,name=null}={}){
  ...
  await emit(company.id,session.id,'MISSION_ACCEPTED','Request accepted. I’m turning it into the first concrete result.',{missionId:mission.id,contractId:contract.id});
...
async function persistPublicEvidence(companyId,sessionId,built){
  ...
  if(saved.length)await emit(companyId,sessionId,'EVIDENCE_PERSISTED',`${saved.length} grounded source${saved.length===1?'':'s'} saved.`,{count:saved.length});
  return saved;
}

async function persistArtifact({companyId,session,mission,contract,built,version,previousArtifactId=null,source}){
  await emit(companyId,session.id,'PERSISTING_RESULT','The result is ready. Saving the artifact and evidence now.',{version});
  ...
  const artifact=record('artifact',{
    ...
    deliverableVersion:version,previousArtifactId,qa:{passed:!built?.degraded,semanticVerified:!built?.degraded,provisional:true},source,
  ...
  session.state='active';Object.assign(session.data,{currentStage:'producing_progress',state:'ready',lastProgressAt:now(),firstArtifactId:session.data.firstArtifactId||artifact.id,latestArtifactId:artifact.id,firstValueReadyAt:session.data.firstValueReadyAt||now(),latestJobId:job.id,evidenceCount:evidence.length,resultQuality:built?.degraded?'degraded':'ready'});Object.assign(session,await put(session,{expectedVersion:session.version}));
  await emit(companyId,session.id,built?.degraded?'RESULT_PARTIAL':'RESULT_READY',built?.degraded?'A partial result is ready. I’m showing exactly what worked and what did not.':'First useful result is ready.',{artifactId:artifact.id,version,evidenceCount:evidence.length,degraded:Boolean(built?.degraded)});
  await audit({company_id:companyId,actor:'system',action:version===1?'artifact.first_value_ready':'artifact.revision_ready',subject_kind:'artifact',subject_id:artifact.id,data:{sessionId:session.id,missionId:mission.id,fileCount:files.length,version,evidenceCount:evidence.length,degraded:Boolean(built?.degraded)}});
  return artifact;
}
...
export async function materializeFirstValue(companyId,sessionId){
  ...
  await emit(companyId,sessionId,'FIRST_VALUE_STARTED','Starting the first useful result now.',{});
  let heartbeat=null;
  try{
    heartbeat=setInterval(()=>emit(companyId,sessionId,'STILL_WORKING','Still working — reasoning/research is in progress.',{}).catch(()=>{}),5000);
    const built=await buildInstantValue({request:session.data.goal,context:session.data.context||{},onProgress:async progress=>{if(!progress?.type)return;await emit(companyId,sessionId,progress.type,progress.message||progress.type,progress.data||{})}});
    return await persistArtifact({companyId,session,mission,contract,built,version:1,source:'thin-product-first-value'});
  }catch(error){
    await emit(companyId,sessionId,'FIRST_VALUE_DEGRADED','The primary reasoning path failed. Returning the strongest truthful fallback instead.',{error:String(error?.message||error)}).catch(()=>{});
...
export async function reviseValueMission(companyId,sessionId,message){
  ...
  await emit(companyId,sessionId,'REVISION_STARTED','Applying your update to the current result.',{message:text.slice(0,180)});
  ...
  let built;try{built=await buildInstantValue({request,context:{...(session.data.context||{}),previousArtifact:prior?.data?.files?.map(f=>({name:f.name,content:f.content})).slice(0,3)||[]},onProgress:async progress=>{if(progress?.type)await emit(companyId,sessionId,progress.type,progress.message||progress.type,progress.data||{})}})}catch{built=fallbackArtifact(session.data.goal,{revisionNote:text,previousFiles:prior?.data?.files||[]})}
