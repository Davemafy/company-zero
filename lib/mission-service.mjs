...
async function persistArtifact({companyId,session,mission,contract,built,version,previousArtifactId=null,source}){
  ...
  const artifact=record('artifact',{
    ...
    deliverableVersion:version,previousArtifactId,readiness,claimVerification:built?.claimVerification||null,qa:{passed:ready,semanticVerified:ready,claimVerified:verificationPassed,provisional:!ready},source,
  ...
  await emit(companyId,session.id,ready?'RESULT_READY':'RESULT_PARTIAL',readyMessage,{artifactId:artifact.id,version,evidenceCount:evidence.length,degraded:Boolean(built?.degraded),readiness,claimVerified:verificationPassed,fileNames:files.map(f=>f.name)});
