...
import {normalizeProviderInput,normalizeCapability,invokeCapability,isSideEffecting} from './capabilities.mjs';
...
export async function createCompany(input){
  ...
  company.data.missionId=mission.id; company.data.mode=input.mode||'operating'; company.data.deliverableType=input.deliverableType||null; await put(company); await put(mission); await audit({company_id:company.id,actor:'user',action:'company.created',subject_kind:'company',subject_id:company.id,data:{missionId:mission.id,mode:company.data.mode}});
  if(company.data.mode==='project')await registerProvider(company.id,studioProviderInput());
  return hydrateCompany(await get(company.id));
}
...
export async function executeJob(companyId,jobId,{shadow=false,revisionId=null}={}){
  ...
  run.data.latencyMs+=Date.now()-started;
  if(!failed&&!uncertain&&value?.artifact?.files?.length){
    const existing=(await list({companyId,kind:'artifact',limit:100})).find(x=>x.data?.runId===run.id);
    const artifact=existing||record('artifact',{runId:run.id,jobId:job.id,type:value.artifact.type||'general',title:value.artifact.title||'Deliverable',summary:value.artifact.summary||'',files:value.artifact.files,qa:value.qa||null},companyId,'ready');
    if(!existing)await put(artifact);
    value={...value,artifact:{...value.artifact,id:artifact.id}};
    run.data.artifactId=artifact.id;
  }
  run.data.output=failed?null:value;run.data.status=uncertain?'uncertain':failed?'failed':'completed';run.data.completedAt=now();run.data.pendingApprovalId=null;run.state=run.data.status;Object.assign(run,await put(run,{expectedVersion:run.version}));job.state=run.state;job.data.status=run.state;job.data.runId=run.id;job.data.pendingApprovalId=null;Object.assign(job,await put(job,{expectedVersion:job.version}));const postRecords=await list({companyId,limit:1000});const ev=await evaluate(company,run,postRecords);await emitExternalObservability({company,org,job,run,records:await list({companyId,limit:1000}),evaluation:ev});return run;
}
...
function roleNameForGroup(group,i,count){const names=group.map(x=>x.data?.name);if(names.includes('understand_brief'))return'Brief Strategist';if(names.includes('build_deliverable'))return'Builder';if(names.includes('quality_review'))return'Quality Reviewer';return i===0?'Intake & Context':i===count-1?'Action & Delivery':`Decision ${i}`}
