...
export async function synthesize(companyId,context={}){
  ...
  organizationProposal=await reasoningProposal({task:'Organization Synthesizer',instructions:'Propose a compact organization for the supplied selected strategy. Return operatingFunctions as an array of 1 to 6 short function names that describe real mission responsibilities. Names must be understandable to a user and derived from the mission and selected strategy. Avoid implementation placeholders such as "Frame test", "Run test", "Agent 1", "Worker", or "Generalist". Do not invent capabilities, execution results, people, or external facts.',input:{goalContract:context.goalContract||mission.data.goalContract,worldModel:context.worldModel||null,selectedStrategy:context.strategy||null,capabilities:caps.map(x=>({id:x.id,name:x.data.name,operationKind:x.data.operationKind,risk:x.data.risk,observes:x.data.observes,changes:x.data.changes}))}},'tensormux_organization_synthesis_failed');if(Array.isArray(organizationProposal?.value?.operatingFunctions)&&organizationProposal.value.operatingFunctions.length)functions=organizationProposal.value.operatingFunctions.map(String).slice(0,6)
  const vague=/^(frame test|run test|measure change|agent\s*\d*|worker|generalist)$/i;functions=functions.filter(x=>String(x).trim()&&!vague.test(String(x).trim()));
  if(!functions.length){const hasChange=caps.some(x=>['change','human'].includes(x.data.operationKind)||['write','financial','external_side_effect'].includes(x.data.risk)||x.data.changes?.length),hasVerification=caps.some(x=>x.data.operationKind==='verify'||x.data.measurements?.some(m=>m.externalRefPath));functions=['World Intelligence',...(hasChange?['Outcome Intervention']:[]),...(hasVerification?['Outcome Verification']:[])];}
...
export async function executeJob(companyId,jobId,{shadow=false,revisionId=null}={}){
  ...
  for(let ri=startRole;ri<org.data.roles.length;ri++){
    ...
    for(let ci=(ri===startRole?startCap:0);ci<role.capabilityIds.length;ci++){
      ...
      if(invocation?.state==='dispatched'&&!resumableScrapy){failed='side_effect_outcome_uncertain';invocation=await transitionInvocation(invocation,'uncertain',{error:{message:failed}});await trace(run,role,cap,'uncertain',{invocationId:invocation.id,error:failed});break}
