...
export async function createCompany(input){
  const company=record('company',{name:required(input.name,'name'),status:'draft',activeOrganizationRevisionId:null,missionVersion:1,...(input.mode?{mode:String(input.mode)}:{})});
  ...
  if(input.mode==='project')await registerProvider(company.id,studioProviderInput());
