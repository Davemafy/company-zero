...
import {planInvocation,transitionInvocation,updateInvocationProgress,invocationProgress} from './invocation-journal.mjs';
...
import {studioProviderInput} from './studio.mjs';
import {transitionDeliverable,emitRuntimeEvent,produceReadyModelDeliverables} from './outcome-control.mjs';
import {persistDeliverableArtifact} from './deliverable-engine.mjs';
export {DomainError,validateSchema};
const id=()=>crypto.randomUUID(); const now=()=>new Date().toISOString();
const TRANSITIONABLE_DELIVERABLE_STATES=new Set(['ready','producing','executing','verifying']);
...
export async function createCompany(input){
  const company=record('company',{name:required(input.name,'name'),status:'draft',activeOrganizationRevisionId:null,missionVersion:1,...(input.mode?{mode:String(input.mode)}:{}),...(input.ownerSessionId?{ownerSessionId:String(input.ownerSessionId)}:{})});
  ...
  company.data.missionId=mission.id; await put(company); await put(mission); await audit({company_id:company.id,actor:'user',action:'company.created',subject_kind:'company',subject_id:company.id,data:{missionId:mission.id}});
  // Universal/product outcomes get the built-in Studio; low-level platform callers keep explicit provider control.
  if(['project','universal_v1','universal'].includes(String(input.mode||'')))await registerProvider(company.id,studioProviderInput());
...
export async function synthesize(companyId,context={}){
  ...
  const roles=[]; let functions=Array.isArray(context.deliverables)&&context.deliverables.length?[...new Set(context.deliverables.map(d=>String(d.data?.title||'').trim()).filter(Boolean))].slice(0,6):(Array.isArray(context.strategy?.operatingFunctions)&&context.strategy.operatingFunctions.length?context.strategy.operatingFunctions:[]);let organizationProposal=null;
  organizationProposal=await reasoningProposal({task:'Organization Synthesizer',instructions:'Propose a compact organization for the supplied selected strategy. Return operatingFunctions as an array of 1 to 6 concrete responsibility names derived from this mission and strategy. Avoid implementation placeholders such as Frame test, Run test, Worker, Agent, or generic Decision labels. Do not invent capabilities, execution results, people, or external facts.',input:{goalContract:context.goalContract||mission.data.goalContract,worldModel:context.worldModel||null,selectedStrategy:context.strategy||null,deliverables:(context.deliverables||[]).map(d=>({id:d.id,title:d.data?.title,kind:d.data?.kind,description:d.data?.description,requiredModes:d.data?.requiredModes,dependencies:d.data?.dependencyIds,state:d.state})),capabilities:caps.map(x=>({id:x.id,name:x.data.name,operationKind:x.data.operationKind,risk:x.data.risk,observes:x.data.observes,changes:x.data.changes}))}},'tensormux_organization_synthesis_failed');if(Array.isArray(organizationProposal?.value?.operatingFunctions)&&organizationProposal.value.operatingFunctions.length)functions=organizationProposal.value.operatingFunctions.map(String).slice(0,6)
...
export async function executeJob(companyId,jobId,{shadow=false,revisionId=null}={}){
  ...
  const sequence=operationPlan
    ? operationPlan.map((plannedStep,stepIndex)=>{const cap=caps.get(plannedStep.capabilityId);const role=org.data.roles.find(r=>r.id===plannedStep.actorRoleId)||org.data.roles.find(r=>r.capabilityIds?.includes(plannedStep.capabilityId));return{stepIndex,plannedStep,cap,role}})
    : org.data.roles.flatMap((role,ri)=>role.capabilityIds.map((capId,ci)=>({stepIndex:null,plannedStep:null,cap:caps.get(capId),role,ri,ci})));
  const legacyStart=()=>{const sr=Number(run.data.checkpoint?.roleIndex||0),sc=Number(run.data.checkpoint?.capIndex||0);const idx=sequence.findIndex(x=>(x.ri>sr)||(x.ri===sr&&x.ci>=sc));return idx<0?0:idx};
  const startIndex=Number.isInteger(run.data.checkpoint?.stepIndex)?Number(run.data.checkpoint.stepIndex):legacyStart();
  for(let si=startIndex;si<sequence.length;si++){
    if(job.data.payload?.deliverableGraphId)await produceReadyModelDeliverables(companyId,job.data.payload.deliverableGraphId,{goalContract:job.data.payload?.goalContract,selectedStrategy:org.data?.selectedStrategy});
    const slot=sequence[si],plannedStep=slot.plannedStep,cap=slot.cap,role=slot.role;if(!cap||!role){failed='operation_role_or_capability_missing';break}
    const provider=providers.get(cap.data.providerId)?.data;
    const permission=authorize(org,role,cap);if(permission){failed=permission;await trace(run,role,cap,'blocked',{error:permission,stepIndex:si});break}
    const operationInput=plannedStep?.args??value;const deliverableId=plannedStep?.deliverableId||null;if(deliverableId){try{const d=await get(deliverableId);if(d?.kind==='deliverable'){if(['blocked','failed'].includes(d.state))await transitionDeliverable(companyId,deliverableId,'ready',{operationId:`${run.id}:step:${si}`});const refreshed=await get(deliverableId);if(refreshed?.state==='ready')await transitionDeliverable(companyId,deliverableId,'executing',{operationId:`${run.id}:step:${si}`})}}catch{}}
    if(shadow&&(isSideEffecting(cap.data)||provider?.type==='human')){await trace(run,role,cap,'shadow_blocked',{input:operationInput,operation:plannedStep||null,stepIndex:si});run.data.checkpoint={stepIndex:si+1,roleIndex:slot.ri||0,capIndex:(slot.ci||0)+1,value};Object.assign(run,await put(run,{expectedVersion:run.version}));continue}
    const invocationKey=`${job.id}:${org.id}:${role.id}:${cap.id}:step:${si}`;let invocation=(operationPlan||isSideEffecting(cap.data)||provider?.type==='human')?await planInvocation({companyId,jobId:job.id,runId:run.id,capabilityId:cap.id,invocationKey,input:sanitizeForEvidence(operationInput)}):null;
    if(invocation?.state==='confirmed'){value=invocation.data?.output??invocation.output;run.data.checkpoint={stepIndex:si+1,roleIndex:slot.ri||0,capIndex:(slot.ci||0)+1,value};Object.assign(run,await put(run,{expectedVersion:run.version}));continue}
    const resumableScrapy=invocation?.state==='dispatched'&&provider?.adapter==='scrapy-cloud'&&cap.data.risk==='read';
    if(invocation?.state==='dispatched'&&!resumableScrapy){failed='side_effect_outcome_uncertain';invocation=await transitionInvocation(invocation,'uncertain',{error:{message:failed}});await trace(run,role,cap,'uncertain',{invocationId:invocation.id,error:failed,stepIndex:si});break}
    ...
    if(needsApproval){
      ...
      if(approval.state==='pending'){
        run.state='waiting_for_approval';run.data.status='waiting_for_approval';run.data.pendingApprovalId=approval.id;run.data.checkpoint={stepIndex:si,roleIndex:slot.ri||0,capIndex:slot.ci||0,value};run.data.latencyMs+=Date.now()-started;await trace(run,role,cap,'waiting_for_approval',{approvalId:approval.id,stepIndex:si});Object.assign(run,await put(run,{expectedVersion:run.version}));
      ...
      if(approval.state==='rejected'){failed='human_approval_rejected';if(invocation&&['planned','approved'].includes(invocation.state))invocation=await transitionInvocation(invocation,'failed',{error:{message:failed}});await trace(run,role,cap,'blocked',{approvalId:approval.id,error:failed,invocationId:invocation?.id,stepIndex:si});break}
      if(invocation?.state==='planned')invocation=await transitionInvocation(invocation,'approved');
      if(provider?.type==='human'){value={approved:true,decisionReason:approval.decision_reason||'',input:value};if(invocation){invocation=await transitionInvocation(invocation,'dispatched');invocation=await transitionInvocation(invocation,'confirmed',{output:value})}await trace(run,role,cap,'completed',{approvalId:approval.id,output:value,costUsd:0,latencyMs:0,invocationId:invocation?.id,stepIndex:si});run.data.checkpoint={stepIndex:si+1,roleIndex:slot.ri||0,capIndex:(slot.ci||0)+1,value};Object.assign(run,await put(run,{expectedVersion:run.version}));continue}
    }
    let reservationId;
    try{const input=operationInput,operationStartedAt=now();reservationId=await reserveBudget({companyId,sourceRef:`${run.id}:step:${si}`,amountUsd:Number(cap.data.estimatedCost?.usd||0),limitUsd:Number(org.data.policies.dailyBudgetUsd||0)});if(invocation&&!resumableScrapy)invocation=await transitionInvocation(invocation,'dispatched');const result=await invokeHttpCapability(cap.data,provider,input,{idempotencyKey:invocationKey,executionState:invocationProgress(invocation),onProgress:invocation?async progress=>{invocation=await updateInvocationProgress(invocation,sanitizeForEvidence(progress))}:null});await settleBudget(reservationId,'consumed');value=result.output;if(provider?.type==='internal'&&provider?.config?.builtin==='studio'&&value?.artifact?.files){const operatingSessionId=job.data.operatingSessionId||null;const sessionRecord=operatingSessionId?await get(operatingSessionId):null;const artifact=await persistDeliverableArtifact(companyId,{sessionId:operatingSessionId,missionId:job.data.missionId||null,deliverableContractId:sessionRecord?.data?.deliverableContractId||null,title:value.artifact.title,summary:value.artifact.summary,type:value.artifact.type,files:value.artifact.files,jobId:job.id,runId:run.id,qa:value.qa||null,source:'studio'});value={...value,artifact:{...value.artifact,id:artifact.id,deliverableVersion:artifact.data.deliverableVersion,manifest:artifact.data.manifest}}}run.data.costUsd+=result.cost;const evidence=await trace(run,role,cap,'completed',{providerId:providers.get(cap.data.providerId)?.id||cap.data.providerId,providerAdapter:provider?.adapter||'generic-http',actorRoleId:role.id,organizationRevisionId:org.id,strategySelectionId:job.data.payload?.strategySelectionId||null,operation:plannedStep||{capabilityId:cap.id},stepIndex:si,input:sanitizeForEvidence(input),output:sanitizeForEvidence(value),responseMetadata:sanitizeForEvidence(result.metadata||null),startedAt:operationStartedAt,completedAt:now(),sideEffectState:isSideEffecting(cap.data)?'confirmed':'not_applicable',latencyMs:result.latencyMs,costUsd:result.cost,budgetReservationId:reservationId,invocationId:invocation?.id});await persistCapabilityObservations(run,job,cap,provider,value,evidence.id,invocation?.id,result.metadata);if(deliverableId){try{const terminal=plannedStep?.deliverableTerminalState||((cap.data.operationKind==='change'||(cap.data.changes||[]).length)?'executed':(cap.data.operationKind==='verify'||(cap.data.measurements||[]).length)?'verified':'observed');const current=await get(deliverableId);if(current?.kind==='deliverable'){if(current.state==='executing'&&terminal!=='executed')await transitionDeliverable(companyId,deliverableId,'executed',{evidenceIds:[evidence.id],operationId:`${run.id}:step:${si}`});const after=await get(deliverableId);if(after?.kind==='deliverable'&&after.state!==terminal&&['executed','produced','verified','observed'].includes(terminal)){if(after.state==='executed'&&terminal==='verified')await transitionDeliverable(companyId,deliverableId,'verifying',{evidenceIds:[evidence.id]});const refreshed=await get(deliverableId);if(refreshed.state!==terminal)await transitionDeliverable(companyId,deliverableId,terminal,{evidenceIds:[evidence.id],operationId:`${run.id}:step:${si}`})}}}catch{}}if(invocation)invocation=await transitionInvocation(invocation,'confirmed',{output:sanitizeForEvidence(result.output)});run.data.checkpoint={stepIndex:si+1,roleIndex:slot.ri||0,capIndex:(slot.ci||0)+1,value};Object.assign(run,await put(run,{expectedVersion:run.version}))}
    catch(e){if(e.resumable&&invocation?.state==='dispatched'){await trace(run,role,cap,'pending',{error:e.message,invocationId:invocation.id,remoteJobId:e.remoteJobId||invocationProgress(invocation)?.remoteJobId||null,stepIndex:si});throw e}if(reservationId)try{await settleBudget(reservationId,e.uncertainSideEffect?'consumed':'released')}catch{}if(invocation&&invocation.state==='dispatched')try{invocation=await transitionInvocation(invocation,e.uncertainSideEffect?'uncertain':'failed',{error:{message:e.message,details:e.details||null}})}catch{}uncertain=Boolean(e.uncertainSideEffect);failed=uncertain?'side_effect_outcome_uncertain':e.message;await trace(run,role,cap,uncertain?'uncertain':'failed',{error:failed,errorDetails:e.details||null,invocationId:invocation?.id,stepIndex:si});if(deliverableId){try{const d=await get(deliverableId);if(d?.kind==='deliverable'&&TRANSITIONABLE_DELIVERABLE_STATES.has(d.state))await transitionDeliverable(companyId,deliverableId,'failed',{reason:failed})}catch{}}break}
  }
...
async function evaluate(company,run,records){
  ...
  Object.assign(ev,await put(ev));run.data.evaluationId=ev.id;await emitRuntimeEvent(company.id,{sessionId:job?.data?.operatingSessionId||null,missionId:mission?.id||null,type:'RUN_EVALUATED',subjectKind:'evaluation',subjectId:ev.id,data:{passed:verdict.passed,executionSucceeded:run.data.status==='completed'},evidenceIds:[ev.id]});
