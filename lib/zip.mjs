...
function safeZipPath(input){return String(input||'file.bin').replace(/\\/g,'/').split('/').filter(Boolean).filter(x=>x!=='.'&&x!=='..').map(x=>x.replace(/[^a-zA-Z0-9._ -]/g,'-').slice(0,100)).join('/').slice(0,600)||'file.bin'}
export function zipFiles(files=[]){
  const local=[],central=[];let offset=0;const dt=dosDateTime();
  for(const f of files){
    const name=Buffer.from(safeZipPath(f.name),'utf8');const data=Buffer.isBuffer(f.buffer)?f.buffer:f.bytes instanceof Uint8Array?Buffer.from(f.bytes):Buffer.from(String(f.content||''),'utf8');const crc=crc32(data);
