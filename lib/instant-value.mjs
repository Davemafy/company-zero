import {completeJsonWithTensorMux,primaryRuntimeConfigured,tensormuxModel} from './tensormux.mjs';
...
function researchQuery(text,plan){
  const normalized=String(text||'').replace(/\benterpr(?:enuer|eneur)\s*ship\b/ig,'entrepreneurship').replace(/\benterprenuer\b/ig,'entrepreneur').replace(/\bnigerai\b/ig,'Nigeria');
  if(plan.kind==='lead_generation'||plan.kind==='research')return normalized.slice(0,700);
  const location=(text.match(/\b(?:in|for)\s+([A-Z][A-Za-z -]{2,30})(?:[.,]|\s|$)/)||[])[1]||'';
  if(plan.kind==='company_creation'&&/\b(food|snack|snacks|beverage|drink|grocery)\b/i.test(text))return `consumer packaged food snack CPG brand retail pricing competitors grocery ${location}`.trim();
...
async function runPlanner({text,context,basePlan,model,policy,timeoutMs}){const r=await completeJsonWithTensorMux({model,policy,timeoutMs,temperature:0,system:plannerSystem,user:JSON.stringify({request:text,basePlan,context})});return {response:r,plan:normalizePlannerPlan(basePlan,r.json||{})}}

async function planMission(text,context,onProgress){
  const basePlan=compileExecutionPlan(text,context||{});await progress(onProgress,'ORGANIZATION_ASSEMBLED',`Assembled ${basePlan.organization.functions.join(' → ')} for a ${basePlan.kind.replaceAll('_',' ')} mission.`,{organization:basePlan.organization,contract:basePlan.contract});if(!primaryRuntimeConfigured())return basePlan;
...
async function truthGate({text,plan,built,publicEvidence,onProgress}){
  ...
  if(verdict.passed)return {...built,claimVerification:{passed:true,repaired:false,claims:verdict.claims,provider:verdict.provider,model:verdict.model,telemetry:verdict.telemetry||null,verificationStartedAt:verdict.verificationStartedAt||null,verificationCompletedAt:verdict.verificationCompletedAt||null}};
  if(verdict.failed)return partialCandidate(built,'claim_verifier_failed',{claimVerification:{passed:false,failed:true,error:verdict.error||'claim_verifier_failed',claims:verdict.claims||[],rejected:verdict.rejected||[],provider:verdict.provider||null,model:verdict.model||null,telemetry:verdict.telemetry||null,verificationStartedAt:verdict.verificationStartedAt||null,verificationCompletedAt:verdict.verificationCompletedAt||null},publicEvidence});
  const repaired=await repairRejectedClaims({request:text,plan,result:built,verdict,publicEvidence:evidence,onProgress});
  if(!repaired)return partialCandidate(built,'claim_repair_failed',{claimVerification:{passed:false,failed:true,claims:verdict.claims||[],rejected:verdict.rejected||[],provider:verdict.provider||null,model:verdict.model||null,telemetry:verdict.telemetry||null,verificationStartedAt:verdict.verificationStartedAt||null,verificationCompletedAt:verdict.verificationCompletedAt||null},publicEvidence});
  ...
  if(!verdict.passed){await progress(onProgress,'CLAIM_REPAIR_REJECTED','Bounded repair did not clear claim verification. The repaired candidate is not eligible for READY.',{failed:verdict.failed||false,rejected:verdict.rejected||[]});return partialCandidate(repaired,'claim_repair_verification_failed',{claimVerification:{passed:false,failed:Boolean(verdict.failed),claims:verdict.claims||[],rejected:verdict.rejected||[],provider:verdict.provider||null,model:verdict.model||null,telemetry:verdict.telemetry||null,verificationStartedAt:verdict.verificationStartedAt||null,verificationCompletedAt:verdict.verificationCompletedAt||null,repairTelemetry:repaired?.repairTelemetry||null},publicEvidence})}
  return {...repaired,publicEvidence:built.publicEvidence,provisional:built.provisional,degraded:false,claimVerification:{passed:true,repaired:true,claims:verdict.claims,provider:verdict.provider,model:verdict.model,telemetry:verdict.telemetry||null,repairTelemetry:repaired?.repairTelemetry||null,verificationStartedAt:verdict.verificationStartedAt||null,verificationCompletedAt:verdict.verificationCompletedAt||null}};
}
...
export async function buildInstantValue({request,context=null,onProgress=null}={}){
  ...
  let r;try{r=await completeJsonWithTensorMux({model:executionModel,timeoutMs:executionTimeoutMs(),onProgress,system:`You are Company Zero's execution organization. Return strict JSON: {"title":string,"summary":string,"files":[{"name":string,"mimeType":string,"content":string}]}.\nThe original user outcome is the operating contract. Follow executionPlan.organization and executionPlan.contract.\nProduce concrete usable artifacts now, not a description of future work. Cover every requested contract item that can be completed safely. Infer reversible assumptions. Respect stated budgets and constraints. Label estimates. Use publicEvidence only when it actually supports a claim and include URLs for sourced current facts. Never invent research, suppliers, prospects, prices, actions or measurements. Ask for input only if executionPlan.contract.askPolicy permits it AND work is materially blocked. A missing optional detail must not stop reversible work. For company creation, act as venture strategy + brand + finance + go-to-market + verification, not as a prospecting team. Prefer multiple coherent deliverables when the user explicitly requests them.`,user:JSON.stringify({request:text,executionPlan:plan,context,publicEvidence})})}catch(error){await progress(onProgress,'EXECUTION_DEGRADED',`Executor failed (${String(error?.message||error).slice(0,100)}). Keeping the correct organization and mission contract for recovery.`,{error:String(error?.message||error),kind:plan.kind,model:error?.model||executionModel});return usefulFallback(text,plan,{reason:error?.message||'instant_value_failed',publicEvidence,roleTelemetry:error?.roleTelemetry||null,roleTelemetrySummary:error?.roleTelemetrySummary||null})}
  const built=normalizeModelResult(r);built.publicEvidence=publicEvidence?.ok?{queries:publicEvidence.queries,results:(publicEvidence.results||[]).slice(0,8)}:null;
  if(plan.kind==='research'&&built.publicEvidence?.results?.length){
    const sourceLines=built.publicEvidence.results.map((x,i)=>`${i+1}. [${String(x.title||'Source').replace(/\[/g,'(').replace(/\]/g,')')}](${x.url})\n   - Evidence receipt: ${String(x.snippet||'').slice(0,360)}`);
    const sourceContent=`# Research sources\n\n## Evidence used for findings and recommendations\nThese are the public receipts collected for this research pass. They support only the claims their titles/snippets/URLs actually substantiate.\n\n${sourceLines.join('\n\n')}\n`;
    const existing=built.files.findIndex(file=>file.name==='research-sources.md');
    const sourceFile={name:'research-sources.md',mimeType:'text/markdown',content:sourceContent};
    if(existing>=0)built.files[existing]=sourceFile;else built.files.push(sourceFile);
  }
  const fileNames=built.files.map(f=>f.name);await progress(onProgress,'MODEL_RETURNED',`${r.model} returned ${built.files.length} candidate artifact${built.files.length===1?'':'s'}: ${fileNames.join(', ')||'none'}.`,{provider:r.provider||null,model:r.model||null,fileNames,usage:r.usage||null,roleTelemetry:r.roleTelemetry||null});
