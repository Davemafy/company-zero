...
function leadContextGap(text,evidence,plan){
  // Critical invariant: only a prospecting organization may block on prospecting context.
...
function researchQuery(text,plan){
  if(plan.kind==='lead_generation')return text.slice(0,700);
  if(plan.kind==='research')return text.slice(0,700);
  // Research is supporting evidence, not the mission. Never send a multi-paragraph operating brief verbatim to search.
...
function normalizePlannerPlan(basePlan,json){
  ...
  // kind, organization, contract and ask policy are intentionally inherited only from basePlan.
  // A planner may refine the path, but cannot silently change the mission's operating contract.
...
async function runPlanner({text,context,basePlan,model,timeoutMs}){
  const r=await completeJsonWithTensorMux({model,timeoutMs,temperature:0,system:plannerSystem,user:JSON.stringify({request:text,basePlan,context})});
  return {response:r,plan:normalizePlannerPlan(basePlan,r.json||{})};
}

async function planMission(text,context,onProgress){
  const basePlan=compileExecutionPlan(text,context||{});
  await progress(onProgress,'ORGANIZATION_ASSEMBLED',`Assembled ${basePlan.organization.functions.join(' → ')} for a ${basePlan.kind.replaceAll('_',' ')} mission.`,{organization:basePlan.organization,contract:basePlan.contract});
  if(!tensormuxConfigured())return basePlan;

  const fastModel=tensormuxModel('planner');
  const fallbackModel=tensormuxModel('planner_fallback');
  let firstError=null,retryError=null;
  ...
  try{
    const {response,plan}=await runPlanner({text,context,basePlan,model:fastModel,timeoutMs:plannerTimeoutMs()});
    await progress(onProgress,'PLANNER_FINISHED',`Fast planner kept the ${plan.kind.replaceAll('_',' ')} organization and chose ${plan.workUnits.length} execution steps.`,{model:response.model,attempt:1,workUnits:plan.workUnits,expectedOutputs:plan.expectedOutputs,contract:plan.contract});
    return plan;
  }catch(error){
    firstError=error;
    await progress(onProgress,'PLANNER_RETRYING',`Fast planner attempt 1 failed (${String(error?.message||error).slice(0,100)}). Retrying once with a bounded ${plannerRetryTimeoutMs()}ms window.`,{model:error?.model||fastModel,attempt:1,error:String(error?.message||error),nextAttempt:2,timeoutMs:plannerRetryTimeoutMs(),contract:basePlan.contract});
  }

  try{
    const {response,plan}=await runPlanner({text,context,basePlan,model:fastModel,timeoutMs:plannerRetryTimeoutMs()});
    await progress(onProgress,'PLANNER_FINISHED',`Fast planner retry succeeded without changing the ${plan.kind.replaceAll('_',' ')} organization.`,{model:response.model,attempt:2,workUnits:plan.workUnits,expectedOutputs:plan.expectedOutputs,contract:plan.contract});
    return plan;
  }catch(error){
    retryError=error;
    await progress(onProgress,'PLANNER_RETRY_FAILED',`Fast planner retry failed (${String(error?.message||error).slice(0,100)}). Escalating to the fallback planner before deterministic routing.`,{model:error?.model||fastModel,attempt:2,error:String(error?.message||error),fallbackModel,timeoutMs:fallbackPlannerTimeoutMs(),contract:basePlan.contract});
  }
  ...
  try{
    const {response,plan}=await runPlanner({text,context,basePlan,model:fallbackModel,timeoutMs:fallbackPlannerTimeoutMs()});
    await progress(onProgress,'PLANNER_FALLBACK_SUCCEEDED',`Fallback planner recovered the mission with ${plan.workUnits.length} execution steps and the original ${plan.kind.replaceAll('_',' ')} organization intact.`,{model:response.model,workUnits:plan.workUnits,expectedOutputs:plan.expectedOutputs,contract:plan.contract});
    return plan;
  }catch(error){
    await progress(onProgress,'PLANNER_FALLBACK_FAILED',`Fallback planner failed (${String(error?.message||error).slice(0,100)}). Only now switching to the deterministic plan.`,{model:error?.model||fallbackModel,error:String(error?.message||error),firstError:String(firstError?.message||firstError||''),retryError:String(retryError?.message||retryError||''),contract:basePlan.contract});
    await progress(onProgress,'PLANNER_DEGRADED',`All model planning attempts failed. Preserving the deterministic ${basePlan.kind.replaceAll('_',' ')} organization and the original mission contract.`,{kind:basePlan.kind,organization:basePlan.organization,contract:basePlan.contract});
    return basePlan;
  }
}
...
export async function buildInstantValue({request,context=null,onProgress=null}={}){
  ...
  const executionModel=tensormuxModel('execution');await progress(onProgress,'EXECUTION_STARTED',`Executor (${executionModel}) is producing artifacts for: ${plan.workUnits[0]?.title||plan.objective}`,{model:executionModel,workUnits:plan.workUnits,timeoutMs:executionTimeoutMs()});
  let r;try{r=await completeJsonWithTensorMux({model:executionModel,timeoutMs:executionTimeoutMs(),system:`You are Company Zero's execution organization. Return strict JSON: {"title":string,"summary":string,"files":[{"name":string,"mimeType":string,"content":string}]}.
...
Produce concrete usable artifacts now, not a description of future work. Cover every requested contract item that can be completed safely. Infer reversible assumptions. Respect stated budgets and constraints. Label estimates. Use publicEvidence only when it actually supports a claim and include URLs for sourced current facts. Never invent research, suppliers, prospects, prices, actions or measurements. Ask for input only if executionPlan.contract.askPolicy permits it AND work is materially blocked. A missing optional detail must not stop reversible work. For company creation, act as venture strategy + brand + finance + go-to-market + verification, not as a prospecting team. Prefer multiple coherent deliverables when the user explicitly requests them.`,user:JSON.stringify({request:text,executionPlan:plan,context,publicEvidence})})}catch(error){await progress(onProgress,'EXECUTION_DEGRADED',`Executor failed (${String(error?.message||error).slice(0,100)}). Keeping the correct organization and mission contract for recovery.`,{error:String(error?.message||error),kind:plan.kind,model:error?.model||executionModel});return usefulFallback(text,plan,{reason:error?.message||'instant_value_failed',publicEvidence})}
  const built=normalizeModelResult(r);const fileNames=built.files.map(f=>f.name);await progress(onProgress,'MODEL_RETURNED',`${r.model} returned ${built.files.length} candidate artifact${built.files.length===1?'':'s'}: ${fileNames.join(', ')||'none'}.`,{model:r.model||null,fileNames,usage:r.usage||null});built.publicEvidence=publicEvidence?.ok?{queries:publicEvidence.queries,results:(publicEvidence.results||[]).slice(0,8)}:null;
  const qa=qualityCheck(built,plan);await progress(onProgress,'EVALUATOR_JUDGMENT',qa.ok?`Evaluator: PASS — artifacts satisfy the current mission contract.`:`Evaluator: FAIL — ${qa.reasons.join(', ')}.`,{reasons:qa.reasons,contract:qa.contract,structuralFailure:qa.structuralFailure,organization:plan.organization});
  if(!qa.ok){if(qa.structuralFailure)await progress(onProgress,'STRUCTURAL_FAILURE',`The current organization failed the outcome contract. Recording this as structural evidence rather than calling the mission complete.`,{organization:plan.organization,reasons:qa.reasons,contract:qa.contract});return usefulFallback(text,plan,{reason:`quality:${qa.reasons.join(',')}`,publicEvidence})}
  await progress(onProgress,'RESULT_COMPOSED',`Evaluator approved ${built.files.length} artifact${built.files.length===1?'':'s'}: ${fileNames.join(', ')}.`,{fileCount:built.files.length,kind:plan.kind,fileNames,model:r.model,organization:plan.organization});return built;
}
