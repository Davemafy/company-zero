...
const plannerTimeoutMs=()=>Math.max(2000,Math.min(10000,Number(process.env.PLANNER_TIMEOUT_MS||7000)));
...
function normalizePlannerPlan(basePlan,json){const units=Array.isArray(json?.workUnits)?json.workUnits.map(x=>String(x||'').trim()).filter(Boolean).slice(0,7):[];const outputs=Array.isArray(json?.expectedOutputs)?json.expectedOutputs.map(x=>safeName(x)).filter(Boolean).slice(0,6):[];return {...basePlan,objective:String(json?.objective||basePlan.objective).trim().slice(0,500)||basePlan.objective,workUnits:(units.length?units:basePlan.workUnits.map(x=>x.title)).map((title,index)=>({id:index+1,title,state:index===0?'active':'queued'})),expectedOutputs:outputs.length?outputs:basePlan.expectedOutputs,requiresFreshEvidence:typeof json?.requiresFreshEvidence==='boolean'?json.requiresFreshEvidence:basePlan.requiresFreshEvidence,plannerRationale:String(json?.reason||'').trim().slice(0,500)||null}}

async function planMission(text,context,onProgress){
  const basePlan=compileExecutionPlan(text,context||{});await progress(onProgress,'ORGANIZATION_ASSEMBLED',`Assembled ${basePlan.organization.functions.join(' → ')} for a ${basePlan.kind.replaceAll('_',' ')} mission.`,{organization:basePlan.organization,contract:basePlan.contract});
  if(!tensormuxConfigured())return basePlan;const plannerModel=tensormuxModel('planner');await progress(onProgress,'PLANNER_STARTED',`Planner (${plannerModel}) is testing the shortest useful path against the mission contract.`,{model:plannerModel,timeoutMs:plannerTimeoutMs()});
  try{const r=await completeJsonWithTensorMux({model:plannerModel,timeoutMs:plannerTimeoutMs(),temperature:0,system:'You are Company Zero mission planner. Return strict JSON only: {"objective":string,"workUnits":string[],"expectedOutputs":string[],"requiresFreshEvidence":boolean,"reason":string}. Preserve the user outcome, constraints, requested deliverables and ask policy. Never turn a venture-building mission into lead generation merely because customers are mentioned. Plan concrete artifacts, not workflow theatre.',user:JSON.stringify({request:text,basePlan,context})});const refined=normalizePlannerPlan(basePlan,r.json||{});await progress(onProgress,'PLANNER_FINISHED',`Planner kept the ${refined.kind.replaceAll('_',' ')} organization and chose ${refined.workUnits.length} execution steps.`,{model:r.model,workUnits:refined.workUnits,expectedOutputs:refined.expectedOutputs,contract:refined.contract});return refined}catch(error){await progress(onProgress,'PLANNER_DEGRADED',`Planner failed (${String(error?.message||error).slice(0,100)}). Preserving the deterministic organization and original mission contract.`,{model:error?.model||plannerModel,error:String(error?.message||error)});return basePlan}
}
