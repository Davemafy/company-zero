...
const timeoutMs=()=>Math.max(8000,Math.min(45000,Number(process.env.INSTANT_VALUE_TIMEOUT_MS||32000)));
...
export async function buildInstantValue({request,context=null,onProgress=null}={}){
  ...
  await progress(onProgress,'EXECUTION_STARTED',`Working on: ${firstUnit}`,{workUnits:plan.workUnits,activeUnit:plan.workUnits[0]||null,timeoutMs:timeoutMs()});
  let r;
  try{
