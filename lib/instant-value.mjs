...
const plannerTimeoutMs=()=>Math.max(2000,Math.min(8000,Number(process.env.PLANNER_TIMEOUT_MS||5000)));
...
function normalizePlannerPlan(basePlan,json){
  const units=Array.isArray(json?.workUnits)?json.workUnits.map(x=>String(x||'').trim()).filter(Boolean).slice(0,6):[];
  const outputs=Array.isArray(json?.expectedOutputs)?json.expectedOutputs.map(x=>safeName(x)).filter(Boolean).slice(0,4):[];
  return {
    ...basePlan,
    objective:String(json?.objective||basePlan.objective).trim().slice(0,500)||basePlan.objective,
    workUnits:(units.length?units:basePlan.workUnits.map(x=>x.title)).map((title,index)=>({id:index+1,title,state:index===0?'active':'queued'})),
    expectedOutputs:outputs.length?outputs:basePlan.expectedOutputs,
    requiresFreshEvidence:typeof json?.requiresFreshEvidence==='boolean'?json.requiresFreshEvidence:basePlan.requiresFreshEvidence,
    plannerRationale:String(json?.reason||'').trim().slice(0,500)||null
  };
}
...
async function planMission(text,context,onProgress){
  ...
  const plannerModel=tensormuxModel('planner');
  await progress(onProgress,'PLANNER_STARTED',`Fast planner (${plannerModel}) is shaping the shortest useful execution path.`,{model:plannerModel,timeoutMs:plannerTimeoutMs()});
  try{
    const r=await completeJsonWithTensorMux({
      model:plannerModel,
      timeoutMs:plannerTimeoutMs(),
      temperature:0,
      system:'You are Company Zero\'s fast mission planner. Return strict JSON only: {"objective":string,"workUnits":string[],"expectedOutputs":string[],"requiresFreshEvidence":boolean,"reason":string}. Keep 2-5 work units. Plan for the first concrete useful result, not the entire future company. Infer reversible assumptions. Do not claim external actions happened.',
      user:JSON.stringify({request:text,basePlan,context})
    });
    const refined=normalizePlannerPlan(basePlan,r.json||{});
    await progress(onProgress,'PLANNER_FINISHED',`Planner chose ${refined.workUnits.length} step${refined.workUnits.length===1?'':'s'} using ${r.model}.`,{model:r.model,workUnits:refined.workUnits,expectedOutputs:refined.expectedOutputs,requiresFreshEvidence:refined.requiresFreshEvidence});
    return refined;
  }catch(error){
    await progress(onProgress,'PLANNER_DEGRADED',`Fast planner failed (${String(error?.message||error).slice(0,120)}). Using the deterministic plan instead.`,{model:error?.model||plannerModel,error:String(error?.message||error)});
    return basePlan;
  }
}
...
export async function buildInstantValue({request,context=null,onProgress=null}={}){
  ...
  const plan=await planMission(text,context,onProgress);
  await progress(onProgress,'PLAN_COMPILED',`Plan: ${plan.workUnits.map((u,i)=>`${i+1}) ${u.title}`).join(' → ')}`,{kind:plan.kind,objective:plan.objective,workUnits:plan.workUnits,expectedOutputs:plan.expectedOutputs,requiresFreshEvidence:plan.requiresFreshEvidence});
  ...
  const executionModel=tensormuxModel('execution');
  const firstUnit=plan.workUnits[0]?.title||plan.objective;
  await progress(onProgress,'EXECUTION_STARTED',`Executor (${executionModel}) is working on: ${firstUnit}`,{model:executionModel,workUnits:plan.workUnits,activeUnit:plan.workUnits[0]||null,timeoutMs:executionTimeoutMs()});
  let r;
  try{
...
- Prefer 1-3 substantial files that a user can actually use.`,
    ...
    await progress(onProgress,'EXECUTION_DEGRADED',`Executor ${error?.model||executionModel} failed (${String(error?.message||error).slice(0,120)}). Switching to the domain fallback instead of stalling.`,{error:String(error?.message||error),kind:plan.kind,model:error?.model||executionModel});
  ...
  await progress(onProgress,'MODEL_RETURNED',`${r.model} returned ${built.files.length} candidate file${built.files.length===1?'':'s'}${fileNames.length?`: ${fileNames.join(', ')}`:''}.`,{model:r.model||null,fileNames,usage:r.usage||null});
  ...
  await progress(onProgress,'RESULT_COMPOSED',`Ready to save ${built.files.length} usable file${built.files.length===1?'':'s'}: ${fileNames.join(', ')}.`,{fileCount:built.files.length,kind:plan.kind,fileNames,model:r.model});
  return built;
}
