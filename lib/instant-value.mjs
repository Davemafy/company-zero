...
async function truthGate({text,plan,built,publicEvidence,onProgress}){
  ...
  if(verdict.failed)return usefulFallback(text,plan,{reason:'claim_verifier_failed',publicEvidence});
  const repaired=await repairRejectedClaims({request:text,plan,result:built,verdict,publicEvidence:evidence,onProgress});
  if(!repaired)return usefulFallback(text,plan,{reason:'claim_repair_failed',publicEvidence});
  ...
  if(!repairedQa.ok)return usefulFallback(text,plan,{reason:`repair_quality:${repairedQa.reasons.join(',')}`,publicEvidence});
  verdict=await verifyArtifactClaims({request:text,plan,result:repaired,publicEvidence:evidence,onProgress});
  if(!verdict.passed){await progress(onProgress,'CLAIM_REPAIR_REJECTED','Bounded repair did not clear claim verification. The repaired candidate is not eligible for READY.',{failed:verdict.failed||false,rejected:verdict.rejected||[]});return usefulFallback(text,plan,{reason:'claim_repair_verification_failed',publicEvidence})}
