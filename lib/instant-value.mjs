...
const timeoutMs=()=>Math.max(2500,Math.min(10000,Number(process.env.INSTANT_VALUE_TIMEOUT_MS||6000)));
...
function fallbackInstantValue(text,reason='model_unavailable'){
  ...
  return {title:`First artifact — ${short}`,summary:'Immediate durable recovery artifact while deeper work continues.',files:[{name:'first-artifact.md',mimeType:'text/markdown',content}],provisional:true,degraded:true,degradedReason:String(reason||'model_unavailable'),model:null};
}
