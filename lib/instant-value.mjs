...
async function collectEvidence(text,context,onProgress){
  const route=String(context?.interactionRoute||'');const needsFreshEvidence=Boolean(context?.needsFreshEvidence)||route==='investigate';
  if(!needsFreshEvidence){await progress(onProgress,'ASSUMPTIONS_SET','No fresh web evidence is required for this first pass. I’m using reversible assumptions.',{});return null;}
  await progress(onProgress,'RESEARCH_STARTED','Searching for current public evidence relevant to the request.',{});
  try{
    const result=await searchPublicWeb(text,{limit:8,timeoutMs:1800});
    ...
    await progress(onProgress,'RESEARCH_DEGRADED','Public research failed; I’m continuing without pretending evidence exists.',{error:String(error?.message||error)});
...
export async function buildInstantValue({request,context=null,onProgress=null}={}){
  ...
  await progress(onProgress,'INTERPRETING_REQUEST','Interpreting the outcome and choosing the first useful deliverable.',{});
  const publicEvidence=await collectEvidence(text,context,onProgress);
  ...
  if(!tensormuxConfigured()){await progress(onProgress,'REASONING_UNAVAILABLE','Reasoning service is not configured; returning a truthful fallback.',{});return fallbackInstantValue(text,'instant_value_model_not_configured',publicEvidence);}
  await progress(onProgress,'REASONING_STARTED','Reasoning over the request and available evidence now.',{evidenceCount:publicEvidence?.results?.length||0});
  let r;
...
- Prefer 1-3 immediately useful files.`,
  ...
  catch(error){await progress(onProgress,'REASONING_DEGRADED','The reasoning pass did not finish cleanly. Returning the strongest truthful fallback.',{error:String(error?.message||error)});return fallbackInstantValue(text,error?.message||'instant_value_failed',publicEvidence)}
  await progress(onProgress,'COMPOSING_RESULT','Reasoning finished. Converting it into a usable result now.',{model:r.model||null});
  ...
  if(!files.length){await progress(onProgress,'RESULT_EMPTY','The model returned no usable files. Falling back instead of faking completion.',{});return fallbackInstantValue(text,'instant_value_empty',publicEvidence);}
  await progress(onProgress,'RESULT_COMPOSED',`${files.length} usable file${files.length===1?'':'s'} composed.`,{fileCount:files.length});
