...
export async function list({companyId,kind,limit=100}={}){assertDurable();if(storageMode()==='development-memory')return [...memory.records.values()].filter(x=>(!companyId||x.company_id===companyId)&&(!kind||x.kind===kind)).sort((a,b)=>b.updated_at.localeCompare(a.updated_at)).slice(0,limit).map(x=>structuredClone(x));let q=`select=*&order=updated_at.desc&limit=${Math.min(limit,5000)}`;if(companyId)q+=`&company_id=eq.${companyId}`;if(kind)q+=`&kind=eq.${encodeURIComponent(kind)}`;const r=await fetch(`${URL}/rest/v1/cz_records?${q}`,{headers:headers()});const x=await r.json();if(!r.ok)throw Error(x.message||'storage_read_failed');return x}
