...
export async function invokeHttpCapability(cap,provider,input,{attempt=1,idempotencyKey=null}={}){
  ...
  if(provider.adapter==='zyte'||provider.adapter==='scrapy-cloud'){
    ...
    try{const result=await invoke(cap,input);return{...result,cost:capabilityCost(cap)}}catch(error){const transient=error instanceof TypeError||error.status>=500||TRANSIENT.has(error.code)||TRANSIENT.has(error.cause?.code);if(transient&&attempt<cap.retry.maxAttempts)return invokeHttpCapability(cap,provider,input,{attempt:attempt+1,idempotencyKey});throw error}
  }
