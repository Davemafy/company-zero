...
export function normalizeProviderInput(input={}){
  ...
  if(input.type==='http'){
    ...
    if(!['generic-http','zyte','scrapy-cloud','github-repository','public-http-probe'].includes(provider.adapter))throw new DomainError('unsupported_provider_adapter',422);
  }
  return provider;
}
...
export async function invokeHttpCapability(cap,provider,input,{attempt=1,idempotencyKey=null,executionState=null,onProgress=null}={}){
  ...
  if(['zyte','scrapy-cloud','github-repository','public-http-probe'].includes(provider.adapter)){
    let invoke,options;
    ...
    else {invoke=(await import('./public-http-probe.mjs')).invokePublicHttpProbeCapability;options=undefined}
