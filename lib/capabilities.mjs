...
export function normalizeProviderInput(input={}){
  ...
  if(!['http','mcp','human','internal'].includes(input.type))throw new DomainError('unsupported_provider_type');
  ...
  if(input.type==='internal'){provider.adapter=input.adapter||'internal';provider.config={...(input.config||{})}}
  if(input.type==='http'){
    ...
    if(!['generic-http','zyte','scrapy-cloud','github-repository','public-http-probe','pagespeed','vercel-deployment'].includes(provider.adapter))throw new DomainError('unsupported_provider_adapter',422);
  }
  return provider;
}
...
export function normalizeCapability(providerId,raw={}){
  ...
  return {
    ...
    stateDomains:Array.isArray(raw.stateDomains)?raw.stateDomains.map(String):[],
    effectModalities:Array.isArray(raw.effectModalities)?raw.effectModalities.map(String):[],
    internalAction:raw.internalAction?String(raw.internalAction):null,
    operationKind,
...
export async function invokeHttpCapability(cap,provider,input,{attempt=1,idempotencyKey=null,executionState=null,onProgress=null}={}){
  ...
  if(provider?.type==='internal'){
    if(provider.adapter!=='internal'&&provider.config?.builtin!=='studio')throw new DomainError('internal_provider_not_supported',422);
    const result=await (await import('./studio.mjs')).invokeStudioCapability(cap,input,{mission:null});return{...result,cost:capabilityCost(cap)}
  }
  if(provider?.type!=='http'||!provider.baseUrl||!cap.endpoint)throw new DomainError('provider_not_executable',422);
  if(['zyte','scrapy-cloud','github-repository','public-http-probe','pagespeed','vercel-deployment'].includes(provider.adapter)){
    let invoke,options;
    if(provider.adapter==='zyte'){invoke=(await import('./zyte.mjs')).invokeZyteCapability;options=undefined}
    else if(provider.adapter==='scrapy-cloud'){invoke=(await import('./scrapy-cloud.mjs')).invokeScrapyCloudCapability;options={idempotencyKey,executionState,onProgress}}
    else if(provider.adapter==='github-repository'){invoke=(await import('./github-repository.mjs')).invokeGitHubRepositoryCapability;options={idempotencyKey}}
    else if(provider.adapter==='public-http-probe'){invoke=(await import('./public-http-probe.mjs')).invokePublicHttpProbeCapability;options=undefined}
    else if(provider.adapter==='pagespeed'){invoke=(await import('./pagespeed.mjs')).invokePageSpeedCapability;options=undefined}
    else {invoke=(await import('./vercel-deployment.mjs')).invokeVercelDeploymentCapability;options=undefined}
    try{const result=await invoke(cap,input,options);return{...result,cost:capabilityCost(cap)}}catch(error){const transient=error instanceof TypeError||error.status>=500||TRANSIENT.has(error.code)||TRANSIENT.has(error.cause?.code);if(!['scrapy-cloud','github-repository'].includes(provider.adapter)&&transient&&attempt<cap.retry.maxAttempts)return invokeHttpCapability(cap,provider,input,{attempt:attempt+1,idempotencyKey,executionState,onProgress});throw error}
  }
