...
export function normalizeProviderInput(input={}){
  ...
  const provider={name:String(input.name),type:input.type,baseUrl:null,status:'registered',config:{...(input.config||{})}};
  if(input.type==='http'){
...
export function normalizeCapability(providerId,raw={}){
  ...
  return {
    ...
    internalAction:raw.internalAction||null
  };
}
...
export async function invokeCapability(cap,provider,input,context={}){
  if(provider?.type==='internal'){const {invokeStudioCapability}=await import('./studio.mjs');return invokeStudioCapability(cap,input,context)}
  return invokeHttpCapability(cap,provider,input,context);
}
