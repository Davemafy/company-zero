...
import {primaryRuntimeConfigured} from './tensormux.mjs';
...
async function build(plan,mission){
  const brief=plan?.brief||plan?.goalContract?.desiredState||plan?.goalContract?.originalRequest||mission?.outcome||'Create the requested deliverable';
...
async function review(input){
  ...
  for(let attempt=0;;attempt++){
    try{
      ...
      if(error?.message!=='deliverable_semantic_quality_failed'||attempt>=maxRevisions||!primaryRuntimeConfigured())throw error;
...
function missingModel(){throw new DomainError('studio_model_not_configured',503,{hint:'Configure Gemini to produce project deliverables.'})}
