...
import {assessOutcomeClosure} from './outcome-modality.mjs';
...
export function capabilityAssessment(capability){
  ...
  return{capabilityId:capability.id,providerId:data.providerId,name:data.name,description:data.description,operationKind:data.operationKind,observes:clean(data.observes),changes:clean(data.changes),stateDomains:clean(data.stateDomains),effectModalities:clean(data.effectModalities),permissions:clean(data.permissions),risk:data.risk,approvalRequired:Boolean(data.approvalRequired),estimatedCost:data.estimatedCost,reversibility:data.reversibility,measurements:clean(data.measurements),inputSchema:data.inputSchema,outputSchema:data.outputSchema,status:'DECLARED_NOT_YET_INVOKED'};
}
...
export function assessCapabilitySufficiency(goalContract={},capabilities=[]){
  ...
  const closure=assessOutcomeClosure(goalContract,capabilities);
  const missing=[];
  ...
  if(requirements.act.required&&!closure.directOutcomeClosable&&closure.requestedOutcomeModality!=='unknown')missing.push({kind:'actuator',reason:`No connected capability can directly cause the requested ${closure.requestedOutcomeModality} world change.`,modality:closure.requestedOutcomeModality});
