...
async function callRole({role,policy,request,plan,context,publicEvidence,upstream,telemetry}){
  const r=await completeJson({policy,role,timeoutMs:timeoutMs(policy),temperature:0,onTelemetry:call=>telemetry.push(call),system:`You are the ${role.replaceAll('_',' ')} function inside a real operating organization. Work only on your function. Return strict JSON {"summary":string,"findings":string[],"files":[{"name":string,"mimeType":string,"content":string}]}. Produce concrete material another function can use. Respect the mission contract and budget. Every internally chosen business decision—including company name, target customer, positioning, channels, prices, headcount, budgets, timelines, margins, performance goals and launch actions—must be explicitly labelled adjacent to the value as Proposed, Target, Estimate, Scenario, Hypothesis, or Assumption unless supplied evidence directly supports it. Never phrase a proposed choice as an observed fact or completed external action. If using JSON, put an explicit status/label beside each chosen value or group it under clearly named proposed/assumptions/estimates/targets keys. Prefer Markdown unless the mission genuinely benefits from machine-readable JSON. Never invent external facts. Use supplied public evidence only when it supports the claim. Do not narrate workflow.`,user:JSON.stringify({request,missionKind:plan.kind,objective:plan.objective,contract:plan.contract,role,context,publicEvidence,upstream})});
...
export async function executeOrganizationRoles({request,plan,context=null,publicEvidence=null,onProgress=null}={}){
 ...
 const runRole=async(role,upstream=[])=>{
  const primaryPolicy=policyForRole(role),policies=primaryPolicy==='routine_executor'?['routine_executor','frontier_escalation']:primaryPolicy==='specialist_executor'?['specialist_executor','frontier_escalation']:[primaryPolicy];
