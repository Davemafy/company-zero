...
export async function completeJsonWithTensorMux({system,user,timeoutMs:override=null,temperature=0}={}){
  ...
  try{const r=await fetch(`${base()}/chat/completions`,{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${process.env.TENSORMUX_API_KEY}`},body:JSON.stringify({model:model(),temperature,response_format:{type:'json_object'},messages:[{role:'system',content:String(system||'Return strict JSON.')},{role:'user',content:String(user||'{}')}]}),signal:controller.signal});const text=await r.text();let body;try{body=text?JSON.parse(text):{}}catch{body={raw:text.slice(0,5000)}}if(!r.ok)throw Object.assign(Error(`tensormux_http_${r.status}`),{details:body,status:r.status});const content=body?.choices?.[0]?.message?.content??'{}';let json;try{json=JSON.parse(content)}catch{throw Object.assign(Error('tensormux_invalid_json'),{details:{content:String(content).slice(0,5000)}})}return{json,usage:body?.usage||null,id:body?.id||null,model:model()}}
  catch(error){if(error?.name==='AbortError')throw Object.assign(Error('tensormux_timeout'),{status:504});throw error}finally{clearTimeout(timer)}
}
