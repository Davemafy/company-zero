...
import {modelGatewayConfigured,primaryRuntimeConfigured as gatewayPrimaryRuntimeConfigured,modelForPolicy,completeJson,auditWithVerifier} from './model-gateway.mjs';
...
export function primaryRuntimeConfigured(){return gatewayPrimaryRuntimeConfigured()}
...
function policyForModel(model){if(model===modelForPolicy('frontier_escalation'))return 'frontier_escalation';if(model===modelForPolicy('verifier'))return 'verifier';if(model===modelForPolicy('cheap_planner'))return 'cheap_planner';return 'routine_executor'}
