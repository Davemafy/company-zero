...
export async function completeJsonWithTensorMux({system,user,timeoutMs=20000,temperature=0}){
  if(!configured())throw Object.assign(Error('tensormux_not_configured'),{status:503});
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const payload={model:model(),temperature,response_format:{type:'json_object'},messages:[{role:'system',content:system},{role:'user',content:user}]};
    ...
    if(!r.ok)throw Object.assign(Error(`tensormux_http_${r.status}`),{status:502,details:body});
    let content=body?.choices?.[0]?.message?.content??'';
    if(typeof content!=='string')content=JSON.stringify(content);
    let json;try{json=JSON.parse(content)}catch{throw Object.assign(Error('tensormux_invalid_json'),{status:502,details:{content:String(content).slice(0,3000)}})}
    return{json,usage:body?.usage||null,id:body?.id||null,model:model()};
  }catch(e){if(e?.name==='AbortError')throw Object.assign(Error('tensormux_timeout'),{status:504});throw e}finally{clearTimeout(timer)}
}
