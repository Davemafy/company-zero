...
import {completeJsonWithTensorMux,primaryRuntimeConfigured} from './tensormux.mjs';
...
export async function compileWorkSpec({request,mission=null,context=null}={}){
  ...
  if(!primaryRuntimeConfigured())return developmentWorkSpec(text);
...
export async function generateWorkBundle({spec,mission=null}={}){
  ...
  if(!primaryRuntimeConfigured())throw new DomainError('studio_model_not_configured',503,{hint:'Configure TensorMux to generate universal deliverables.'});
  const r=await completeJsonWithTensorMux({
    timeoutMs:45000,
  ...
  return {artifact:normalizeBundle(r.json,spec),_usage:r.usage||null,builder:'gemini-universal'};
}
...
export async function reviseWorkBundle({spec,artifact,review,mission=null,attempt=1}={}){
  ...
  if(!primaryRuntimeConfigured())throw new DomainError('studio_model_not_configured',503,{hint:'Configure TensorMux to revise universal deliverables.'});
  ...
  return {artifact:normalizeBundle(r.json,spec),_usage:r.usage||null,builder:'gemini-universal-revision',attempt};
}
...
export async function reviewWorkBundle({spec,artifact}={}){
  ...
  if(!primaryRuntimeConfigured())return {passed:true,checks,semantic:{skipped:true,reason:'primary_runtime_not_configured'}};
  const r=await completeJsonWithTensorMux({
    timeoutMs:30000,
