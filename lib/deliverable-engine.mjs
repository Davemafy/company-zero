...
export function validateArtifactBundle({type='general',files=[],qa=null}={}){
  ...
  return {passed:true,checks:[...files.map(f=>({id:`file:${f.name}`,passed:true})),{id:'bundle_structure',passed:true}]};
}
...
export async function persistDeliverableArtifact(companyId,{sessionId=null,missionId=null,deliverableContractId=null,artifactKey='primary',title='Company Zero Deliverable',summary='',type='general',files=[],jobId=null,runId=null,qa=null,source='studio'}={}){
  ...
  const prepared=prepareFiles(files);const validation=validateArtifactBundle({type,files:prepared,qa});
  ...
  try{
    for(const file of prepared){
      ...
      if(durable){await uploadObject(storagePath,file.buffer,file.mimeType);uploaded.push(storagePath);await markFileReady(reservation.artifactRevisionId,file.name)}
    }
