...
export async function exportRunToNeatlogs({company,mission,organization,job,run,traces,evaluation,modelAudit,semanticRecords=[]}){
  ...
  const semanticKinds=new Set(['outcome_contract','world_model','strategy_option','strategy_selection','organization_revision','operation_plan','external_observation','outcome_verification','diagnosis','replan_request']);
  for(const item of semanticRecords.filter(x=>semanticKinds.has(x.kind)))children.push({name:item.kind.replaceAll('_',' '),kind:'EVENT',input:{recordId:item.id,state:item.state},output:null,attributes:{'company_zero.record_id':String(item.id),'company_zero.record_kind':item.kind,'company_zero.persisted':'true'}});
  const body={
...
export async function exportSemanticEventToNeatlogs({companyId,event,recordId,state,attributes={}}){if(!configured())return{configured:false,skipped:true,reason:'neatlogs_not_configured'};const body={name:`company-zero-${event}`,project:process.env.NEATLOGS_PROJECT,input:{companyId,recordId},output:{state},attributes:{'neatlogs.session.id':String(companyId||''),'company_zero.record_id':String(recordId||''),'company_zero.event':String(event),...attributes},children:[]};const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10000);try{const response=await fetch(`${base()}/v1/trace`,{method:'POST',headers:{'content-type':'application/json','x-api-key':key()},body:JSON.stringify(body),signal:controller.signal}),text=await response.text();let out;try{out=text?JSON.parse(text):{}}catch{out={}}if(!response.ok)throw Object.assign(Error(`neatlogs_http_${response.status}`),{details:out});return{configured:true,skipped:false,...out}}finally{clearTimeout(timer)}}
