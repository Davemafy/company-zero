...
export async function verifyArtifactClaims({request,plan,result,publicEvidence=null,onProgress=null,onTelemetry=null}={}){
  ...
  try{
    ...
    const fallbackRoute=routeForPolicy('verifier_fallback');
    const callVerifier=({policy='verifier',model,structured=true,role='claim_verifier'})=>completeJson({policy,role,model,timeoutMs:timeoutMs(),temperature:0,onTelemetry,...(structured?{responseSchema:CLAIM_VERDICT_SCHEMA,responseSchemaName:'company_zero_claim_verdict'}:{}),system:verifierSystem,user:verifierUser});
    const useProviderFallback=async(errorCode)=>{
      await progress(onProgress,{type:'CLAIM_VERIFIER_RETRYING',message:`Verifier provider fallback · ${fallbackRoute.provider}/${fallbackRoute.model} after ${errorCode}`,data:{errorCode,from:{provider:route.provider,model:route.model},to:{provider:fallbackRoute.provider,model:fallbackRoute.model,format:'json_object'}}});
      return callVerifier({policy:'verifier_fallback',model:fallbackRoute.model,structured:false,role:'claim_verifier_provider_fallback'});
    };
    let r;
    try{
      ...
      if(firstCode==='groq_http_400'){
        ...
        try{r=await callVerifier({model:route.model,structured:false,role:'claim_verifier_json_retry'})}
          ...
          r=await useProviderFallback(compatCode);
        }
      }else if(firstCode==='groq_http_429'){
        r=await useProviderFallback(firstCode);
      }else throw first;
    }
