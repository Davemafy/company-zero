...
const normalizeVerdict=json=>{
  const claims=(Array.isArray(json?.claims)?json.claims:[]).slice(0,40).map((x,i)=>({id:String(x?.id||`claim-${i+1}`),claim:String(x?.claim||'').slice(0,1000),file:String(x?.file||''),status:['SUPPORTED','ASSUMPTION','UNSUPPORTED'].includes(String(x?.status||'').toUpperCase())?String(x.status).toUpperCase():'UNSUPPORTED',reason:String(x?.reason||'').slice(0,1000),support:Array.isArray(x?.support)?x.support.map(String).slice(0,5):[]}));
...
export async function verifyArtifactClaims({request,plan,result,publicEvidence=null,onProgress=null,onTelemetry=null}={}){
  const route=routeForPolicy('verifier');
  await progress(onProgress,{type:'CLAIM_VERIFIER_STARTED',message:`Verifier started · ${route.provider}/${route.model} · checking numeric, current and external claims`,data:{provider:route.provider,model:route.model,policy:'verifier'}});
  try{
    const r=await completeJson({policy:'verifier',role:'claim_verifier',timeoutMs:timeoutMs(),temperature:0,onTelemetry,system:`You are Company Zero's independent claim verifier. Inspect the candidate artifacts claim by claim. Return strict JSON {"passed":boolean,"summary":string,"claims":[{"id":string,"claim":string,"file":string,"status":"SUPPORTED"|"ASSUMPTION"|"UNSUPPORTED","reason":string,"support":string[]}]}. Focus on numeric claims, prices, budgets, margins, market/customer facts, current facts, named external entities, product capabilities, performance claims, and claims that an external action occurred. A claim is SUPPORTED only when supplied evidence directly supports it or it is explicitly supplied by the user. A proposed target, estimate, scenario, hypothesis or assumption is ASSUMPTION only when the artifact labels that specific claim as such. Do not let a generic assumptions section excuse unrelated definitive claims. If a material factual claim is neither supported nor locally labelled as an assumption, mark it UNSUPPORTED. Do not invent support.`,user:JSON.stringify({request,mission:{kind:plan?.kind,contract:plan?.contract,truthPolicy:plan?.truthPolicy},candidateFiles:filesOf(result),publicEvidence})});
    const verdict=normalizeVerdict(r.json||{});
    await progress(onProgress,{type:verdict.passed?'CLAIM_VERIFIER_PASSED':'CLAIM_VERIFIER_REJECTED',message:verdict.passed?`Verifier passed claim-level truth check · ${r.provider}/${r.model}`:`Verifier rejected ${verdict.rejected.length} unsupported claim${verdict.rejected.length===1?'':'s'} · ${r.provider}/${r.model}`,data:{provider:r.provider,model:r.model,policy:r.policy,latencyMs:r.latencyMs,costUsd:r.costUsd,rejected:verdict.rejected,claims:verdict.claims}});
    return {...verdict,telemetry:r.telemetry,provider:r.provider,model:r.model};
  }catch(error){await progress(onProgress,{type:'CLAIM_VERIFIER_FAILED',message:`Verifier failed · ${error.provider||route.provider}/${error.model||route.model}. Result cannot become READY.`,data:{error:String(error?.message||error),telemetry:error?.telemetry||null}});return {passed:false,failed:true,summary:'claim_verifier_failed',claims:[],rejected:[],error:String(error?.message||error),telemetry:error?.telemetry||null};}
}
...
export async function repairRejectedClaims({request,plan,result,verdict,publicEvidence=null,onProgress=null,onTelemetry=null}={}){
  ...
  const route=routeForPolicy('specialist_executor');
  await progress(onProgress,{type:'CLAIM_REPAIR_STARTED',message:`Repair started · ${route.provider}/${route.model} · fixing ${verdict.rejected.length} rejected claim${verdict.rejected.length===1?'':'s'}`,data:{provider:route.provider,model:route.model,rejected:verdict.rejected}});
  try{
    ...
    const repaired={...result,title:String(r.json?.title||result?.title||'Repaired result').slice(0,120),summary:String(r.json?.summary||result?.summary||'Unsupported claims repaired.').slice(0,500),files,model:r.model,repairTelemetry:r.telemetry};
    await progress(onProgress,{type:'CLAIM_REPAIR_COMPLETED',message:`Repair completed · ${r.provider}/${r.model} · ${files.length} artifact${files.length===1?'':'s'} revised`,data:{provider:r.provider,model:r.model,policy:r.policy,latencyMs:r.latencyMs,costUsd:r.costUsd,fileNames:files.map(f=>f.name)}});
    return repaired;
  }catch(error){await progress(onProgress,{type:'CLAIM_REPAIR_FAILED',message:`Repair failed · ${error.provider||route.provider}/${error.model||route.model}. Result cannot become READY.`,data:{error:String(error?.message||error),telemetry:error?.telemetry||null}});return null;}
}
