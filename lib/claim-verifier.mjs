...
function inherentlyProposedClaim(claim,{plan=null,request=''}={}){
  ...
  const creationRequest=/\b(make|build|create|start|launch|form)\b[^.]{0,80}\b(company|startup|business|brand|venture)\b|\b(company|startup|business|brand|venture)\b[^.]{0,80}\b(make|build|create|start|launch|form)\b/i.test(String(request||''));
  if(!creationKind&&!creationRequest)return false;
  return /^(company|brand) name\b/.test(norm(claim));
}
...
export async function verifyArtifactClaims({request,plan,result,publicEvidence=null,onProgress=null,onTelemetry=null}={}){
  ...
  try{
    ...
    let r;
    try{
      r=await completeJson({policy:'verifier',role:'claim_verifier',timeoutMs:timeoutMs(),temperature:0,onTelemetry,responseSchema:CLAIM_VERDICT_SCHEMA,responseSchemaName:'company_zero_claim_verdict',system:verifierSystem,user:verifierUser});
    }catch(first){
      const firstCode=sanitizeProviderError(first,first.provider||route.provider);
      if(!['groq_http_400','groq_http_429'].includes(firstCode))throw first;
      const fallbackModel=verifierFallbackModel();
      await progress(onProgress,{type:'CLAIM_VERIFIER_RETRYING',message:`Verifier model retry · ${route.provider}/${fallbackModel} after ${firstCode}`,data:{errorCode:firstCode,from:{provider:first.provider||route.provider,model:first.model||route.model},to:{provider:route.provider,model:fallbackModel}}});
      r=await completeJson({policy:'verifier',role:'claim_verifier_retry',model:fallbackModel,timeoutMs:timeoutMs(),temperature:0,onTelemetry,responseSchema:CLAIM_VERDICT_SCHEMA,responseSchemaName:'company_zero_claim_verdict',system:verifierSystem,user:verifierUser});
    }
