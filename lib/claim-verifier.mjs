...
function inherentlyProposedClaim(claim,plan){
  if(String(plan?.kind||'')!=='company_creation')return false;
  const c=norm(claim);
  return /^(company|brand) name\b/.test(c);
}
...
const normalizeVerdict=(json,{request='',publicEvidence=null,files=[],plan=null}={})=>{
  const claims=(Array.isArray(json?.claims)?json.claims:[]).slice(0,40).map((x,i)=>{
    ...
    const locallyLabelled=locallyLabelledAssumption(claim,file,files)||inherentlyProposedClaim(claim,plan);
