...
export class WorkerService{
  ...
  async tick(){
    await heartbeat(this.workerId,null,{status:'polling',leaseSeconds:this.leaseSeconds,retryPolicy:'bounded'});
    ...
    await heartbeat(this.workerId,q.id,{status:'working',queueState:q.state,attempt:queueAttempt(q),maxAttempts:queueMaxAttempts(q),leaseSeconds:this.leaseSeconds});
    ...
    try{
      ...
      clearInterval(timer);try{await heartbeat(this.workerId,null,{status:'idle',leaseSeconds:this.leaseSeconds})}catch{}
    }
  }
}
