...
import{claim,transition,retry,heartbeat,renewLease}from'./queue.mjs';
...
async function experimentSessions(experiment){
  ...
  return [...new Set(sources.map(x=>x?.data?.operatingSessionId||x?.data?.sessionId||x?.data?.payload?.sessionId).filter(Boolean))];
}
...
export class WorkerService{
  ...
  async tick(){
    await heartbeat(this.workerId,null,{status:'polling'});
    let q=await claim(this.workerId,this.leaseSeconds);if(!q)return null;
    await heartbeat(this.workerId,q.id,{status:'working',queueState:q.state});
    let busy=false,renewError=null;
    const timer=setInterval(async()=>{if(busy)return;busy=true;try{await renewLease(q,this.workerId,this.leaseSeconds)}catch(e){renewError=e}try{await heartbeat(this.workerId,q.id,{status:'working',queueState:q.state,leaseRenewedAt:new Date().toISOString()})}catch{}finally{busy=false}},this.renewEveryMs);
    try{
      q=await transition(q,'running',{actor:this.workerId});
      await this.crashHook?.('after_claim',q);
      const jid=q.job_id||q.data.jobId,j=await get(jid);if(!j)throw Error('queued_job_missing');
      ...
      if(j.data.operation==='aggregate_experiment'){
        ...
        if(promotion?.action==='PROMOTE'){
          ...
          for(const sessionId of sessions){
            await emitRuntimeEvent(q.company_id,{sessionId,type:'ORGANIZATION_PROMOTED',subjectKind:'organization_revision',subjectId:promotion.candidateId,data:{message:`Organization promoted — Candidate ${label} proved better on the frozen cases and is now production.`,candidateLabel:label,candidateRevisionId:promotion.candidateId,decisionId:promotion.decisionId,result:promotion.result}}).catch(()=>{});
            await scheduleReplan(q.company_id,sessionId,{reason:'A candidate organization passed the evidence gates and was promoted.',sourceRef:`promotion:${promotion.decisionId}`,activate:true,evidenceIds:[promotion.decisionId]});
          }
        }else{
          for(const sessionId of sessions)await emitRuntimeEvent(q.company_id,{sessionId,type:'GOVERNOR_BASELINE_KEPT',subjectKind:'experiment',subjectId:experiment.id,data:{message:'No candidate cleared every promotion gate. The current production organization stays in place.',experimentId:experiment.id}}).catch(()=>{});
        }
      }
      ...
      q=await retry(q,e.message,{uncertain:Boolean(e.uncertainSideEffect)});return{queue:q,error:e.message};
    }finally{
      clearInterval(timer);try{await heartbeat(this.workerId,null,{status:'idle'})}catch{}
    }
  }
}
