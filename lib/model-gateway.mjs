const POLICIES={
  ...
  specialist_executor:{provider:'vercel_gateway',modelEnv:'AI_GATEWAY_SPECIALIST_MODEL',fallbackModel:'anthropic/claude-sonnet-4.6',strictProvider:true},
  frontier_escalation:{provider:'vercel_gateway',modelEnv:'AI_GATEWAY_FRONTIER_MODEL',fallbackModel:'anthropic/claude-opus-5',strictProvider:true},
  verifier:{provider:'vercel_gateway',modelEnv:'AI_GATEWAY_VERIFIER_MODEL',fallbackModel:'openai/gpt-5.6-sol',strictProvider:true}
};
const now=()=>Date.now();
const num=v=>Number.isFinite(Number(v))?Number(v):0;
const strip=s=>String(s||'').replace(/\/$/,'').replace(/\/v1$/,'');
const providerConfig=provider=>{
  if(provider==='tensormux')return {base:strip(process.env.TENSORMUX_BASE_URL),key:process.env.TENSORMUX_API_KEY,timeout:Number(process.env.TENSORMUX_TIMEOUT_MS||60000)};
  if(provider==='vercel_gateway')return {base:strip(process.env.AI_GATEWAY_BASE_URL||'https://ai-gateway.vercel.sh'),key:process.env.AI_GATEWAY_API_KEY||process.env.VERCEL_OIDC_TOKEN,timeout:Number(process.env.AI_GATEWAY_TIMEOUT_MS||60000),authMode:process.env.AI_GATEWAY_API_KEY?'api_key':process.env.VERCEL_OIDC_TOKEN?'oidc':null};
  return {base:'',key:'',timeout:60000,authMode:null};
};
...
const gatewayFallbackModels=policy=>{
  const envKey=policy==='specialist_executor'?'AI_GATEWAY_SPECIALIST_FALLBACK_MODELS':policy==='frontier_escalation'?'AI_GATEWAY_FRONTIER_FALLBACK_MODELS':policy==='verifier'?'AI_GATEWAY_VERIFIER_FALLBACK_MODELS':null;
  const defaults=policy==='specialist_executor'?['google/gemini-3-flash']:policy==='frontier_escalation'?['openai/gpt-5.6-sol']:policy==='verifier'?['google/gemini-3-flash']:[];
  const raw=envKey?String(process.env[envKey]||'').trim():'';
  return raw?[...new Set(raw.split(',').map(x=>x.trim()).filter(Boolean))]:defaults;
};
...
export function providerConfigSnapshot(){
  const tensor=providerConfig('tensormux'),gateway=providerConfig('vercel_gateway');
  return {
    ...
    vercelGateway:{configured:Boolean(gateway.base&&gateway.key),base:gateway.base||null,authMode:gateway.authMode,specialistModel:defaultModelFor('specialist_executor'),frontierModel:defaultModelFor('frontier_escalation'),verifierModel:defaultModelFor('verifier')}
  };
}
...
function reportedCost(body){for(const value of [body?.usage?.cost,body?.usage?.cost_usd,body?.cost,body?.cost_usd,body?.meta?.cost,body?.meta?.cost_usd,body?.provider_metadata?.gateway?.cost]){const n=Number(value);if(Number.isFinite(n)&&n>=0)return n}return null}
...
function requestBodyFor({provider,policy,role,model,temperature,system,user}){
  const body={model,temperature,messages:[{role:'system',content:String(system||'Return strict JSON.')},{role:'user',content:String(user||'{}')}]};
  if(provider==='tensormux')body.response_format={type:'json_object'};
  if(provider==='vercel_gateway'){
    const models=gatewayFallbackModels(policy);
    body.providerOptions={gateway:{...(models.length?{models}:{}),tags:['company-zero',`policy:${policy}`,`role:${String(role||policy)}`]}};
  }
  return body;
}
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null}={}){
  ...
  if(!['tensormux','vercel_gateway'].includes(chosenProvider))throw Object.assign(Error('provider_not_supported'),{status:503,provider:chosenProvider,requiredProvider:route.provider,policy});
  ...
  try{
    const response=await fetch(`${cfg.base}/v1/chat/completions`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${cfg.key}`},signal:controller.signal,body:JSON.stringify(requestBodyFor({provider:chosenProvider,policy,role,model:chosenModel,temperature,system,user}))});
...
export async function auditWithVerifier({mission,organization,job,run,traces,evaluation,onTelemetry=null}={}){
  if(!providerConfigured('vercel_gateway'))return {configured:false,skipped:true,reason:'vercel_gateway_not_configured'};
