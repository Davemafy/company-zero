...
const circuitKey=(provider,model)=>`${String(provider||'provider')}:${String(model||'default')}`;
function circuitState(provider,model){
  const key=circuitKey(provider,model),state=providerCircuits.get(key);if(!state)return null;
  if(state.until<=now()){providerCircuits.delete(key);return null}
  return state;
}
function tripCircuit(provider,model,code){
  if(/_(?:timeout|network_error|http_429)$/.test(String(code||'')))providerCircuits.set(circuitKey(provider,model),{until:now()+circuitOpenMs(),code});
}
function clearCircuit(provider,model){providerCircuits.delete(circuitKey(provider,model))}
...
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null,responseSchema=null,responseSchemaName='company_zero_payload'}={}){
  ...
  const openCircuit=circuitState(chosenProvider,chosenModel);
