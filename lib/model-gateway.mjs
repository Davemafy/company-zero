...
const agentRouterBase=value=>{const base=strip(value);try{const url=new URL(base);if(['agentrouter.org','www.agentrouter.org'].includes(url.hostname)){const path=url.pathname==='/'?'':url.pathname;return `https://co.agentrouter.org${path}`.replace(/\/$/,'')}}catch{}return base};
const providerConfig=provider=>provider==='tensormux'?{base:strip(process.env.TENSORMUX_BASE_URL),key:process.env.TENSORMUX_API_KEY,timeout:Number(process.env.TENSORMUX_TIMEOUT_MS||60000)}:{base:agentRouterBase(process.env.AGENTROUTER_BASE_URL),key:process.env.AGENTROUTER_API_KEY,timeout:Number(process.env.AGENTROUTER_TIMEOUT_MS||60000)};
