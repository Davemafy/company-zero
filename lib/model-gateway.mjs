const POLICIES={
  cheap_planner:{provider:'gemini',modelEnv:'GEMINI_PRIMARY_MODEL',fallbackModel:'gemini-3.5-flash-lite',strictProvider:true},
  routine_executor:{provider:'gemini',modelEnv:'GEMINI_PRIMARY_MODEL',fallbackModel:'gemini-3.5-flash-lite',strictProvider:true},
  specialist_executor:{provider:'gemini',modelEnv:'GEMINI_PRIMARY_MODEL',fallbackModel:'gemini-3.5-flash-lite',strictProvider:true},
  frontier_escalation:{provider:'openrouter',modelEnv:'OPENROUTER_FALLBACK_MODEL',fallbackModel:'openrouter/free',strictProvider:true},
  verifier:{provider:'groq',modelEnv:'GROQ_VERIFIER_MODEL',fallbackModel:'openai/gpt-oss-120b',strictProvider:true},
  verifier_fallback:{provider:'openrouter',modelEnv:'OPENROUTER_VERIFIER_FALLBACK_MODEL',fallbackModel:'openrouter/free',strictProvider:true}
};
const now=()=>Date.now();
...
const circuitKey=(provider,base,model)=>`${String(provider||'provider')}:${trimBase(base)||'default'}:${String(model||'default')}`;
function circuitState(provider,base,model){
  const key=circuitKey(provider,base,model),state=providerCircuits.get(key);if(!state)return null;
  if(state.until<=now()){providerCircuits.delete(key);return null}
  return state;
}
function tripCircuit(provider,base,model,code){
  if(/_(?:timeout|network_error|http_429)$/.test(String(code||'')))providerCircuits.set(circuitKey(provider,base,model),{until:now()+circuitOpenMs(),code});
}
function clearCircuit(provider,base,model){providerCircuits.delete(circuitKey(provider,base,model))}
...
export function primaryRuntimeConfigured(){return providerConfigured('gemini')}
export function providerConfigSnapshot(){
  ...
  return {
    tensormux:{configured:Boolean(tensor.base&&tensor.key),base:tensor.base||null,legacy:true,plannerModel:String(process.env.TENSORMUX_PLANNER_MODEL||process.env.TENSORMUX_MODEL||'glm-4-7-flash'),executionModel:String(process.env.TENSORMUX_EXECUTION_MODEL||process.env.TENSORMUX_MODEL||'glm-4-7-flash')},
    openrouter:{configured:Boolean(openrouter.base&&openrouter.key),base:openrouter.base||null,fallbackModel:defaultModelFor('frontier_escalation'),verifierFallbackModel:defaultModelFor('verifier_fallback')},
    gemini:{configured:Boolean(gemini.base&&gemini.key),base:gemini.base||null,primaryModel:defaultModelFor('routine_executor'),plannerModel:defaultModelFor('cheap_planner'),specialistModel:defaultModelFor('specialist_executor')},
...
function requestBodyFor({provider,role,model,temperature,system,user,responseSchema=null,responseSchemaName='company_zero_payload'}){
  const response_format=responseSchema
    ?{type:'json_schema',json_schema:{name:String(responseSchemaName||'company_zero_payload').replace(/[^a-zA-Z0-9_-]/g,'_').slice(0,64)||'company_zero_payload',strict:true,schema:responseSchema}}
    :{type:'json_object'};
  const body={model,temperature,response_format,messages:[{role:'system',content:String(system||'Return strict JSON.')},{role:'user',content:String(user||'{}')}]};
  if(provider==='openrouter')body.provider={require_parameters:true};
  if(provider==='groq'&&/^openai\/gpt-oss-/i.test(String(model||''))){body.reasoning_format='hidden';body.reasoning_effort='low';if(/^claim_verifier/.test(String(role||'')))body.max_completion_tokens=4096;}
  return body;
}
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null,responseSchema=null,responseSchemaName='company_zero_payload'}={}){
  ...
  const openCircuit=circuitState(chosenProvider,cfg.base,chosenModel);
  ...
  try{
    ...
    const response=await fetch(chatUrlFor(chosenProvider,cfg.base),{method:'POST',headers,signal:controller.signal,body:JSON.stringify(requestBodyFor({provider:chosenProvider,role,model:chosenModel,temperature,system,user,responseSchema,responseSchemaName}))});
