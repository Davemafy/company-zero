...
const agentRouterClientHeaders=provider=>provider==='agentrouter'?{Originator:'codex_cli_rs',Version:'0.101.0','User-Agent':'codex_cli_rs/0.101.0 (Mac OS 26.0.1; arm64) Apple_Terminal/464'}:{};
...
export function providerConfigSnapshot(){
  const tensor=providerConfig('tensormux'),agent=providerConfig('agentrouter');
  return {
    tensormux:{configured:Boolean(tensor.base&&tensor.key),base:tensor.base||null,executionModel:String(process.env.TENSORMUX_EXECUTION_MODEL||process.env.TENSORMUX_RUNTIME_MODEL||process.env.TENSORMUX_MODEL||'glm-4-7-flash')},
    agentrouter:{configured:Boolean(agent.base&&agent.key),base:agent.base||null,specialistModel:String(process.env.AGENTROUTER_SPECIALIST_MODEL||'glm-5.3'),frontierModel:String(process.env.AGENTROUTER_FRONTIER_MODEL||'claude-opus-5'),verifierModel:String(process.env.AGENTROUTER_VERIFIER_MODEL||'gpt-5.6-sol')}
  };
}
...
const anthropicRoute=(provider,model)=>provider==='agentrouter'&&/^claude-/i.test(String(model||''));
function contentText(body,{anthropic=false}={}){
  const raw=anthropic?body?.content:body?.choices?.[0]?.message?.content;
  if(typeof raw==='string')return raw.trim();
  if(Array.isArray(raw))return raw.map(part=>typeof part==='string'?part:String(part?.text||part?.content||'')).join('').trim();
  return '';
}
function responseShape(body){return {keys:Object.keys(body&&typeof body==='object'?body:{}).slice(0,20),hasChoices:Array.isArray(body?.choices),hasContent:Array.isArray(body?.content),hasId:Boolean(body?.id)}}
...
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null}={}){
  ...
  let body=null;
  try{
    const anthropic=anthropicRoute(chosenProvider,chosenModel);
    const endpoint=anthropic?`${cfg.base}/v1/messages`:`${cfg.base}/v1/chat/completions`;
    const clientHeaders=agentRouterClientHeaders(chosenProvider);
    const headers=anthropic?{'content-type':'application/json',authorization:`Bearer ${cfg.key}`,'x-api-key':cfg.key,'anthropic-version':'2023-06-01',...clientHeaders}:{'content-type':'application/json',authorization:`Bearer ${cfg.key}`,...clientHeaders};
    const requestBody=anthropic?{model:chosenModel,max_tokens:8192,temperature,system:String(system||'Return strict JSON.'),messages:[{role:'user',content:String(user||'{}')}]}:{model:chosenModel,temperature,response_format:{type:'json_object'},messages:[{role:'system',content:String(system||'Return strict JSON.')},{role:'user',content:String(user||'{}')}]};
    const response=await fetch(endpoint,{method:'POST',headers,signal:controller.signal,body:JSON.stringify(requestBody)});
    ...
    const content=contentText(body,{anthropic});if(!content)throw Object.assign(Error(`${chosenProvider}_invalid_response_shape`),{details:responseShape(body)});
