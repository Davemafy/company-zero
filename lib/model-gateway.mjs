const POLICIES={
  ...
  specialist_executor:{provider:'agentrouter',modelEnv:'AGENTROUTER_SPECIALIST_MODEL',fallbackModel:'glm-5.3'},
  frontier_escalation:{provider:'agentrouter',modelEnv:'AGENTROUTER_FRONTIER_MODEL',fallbackModel:'claude-opus-5'},
  verifier:{provider:'agentrouter',modelEnv:'AGENTROUTER_VERIFIER_MODEL',fallbackModel:'gpt-5.6-sol'}
};
const now=()=>Date.now();
...
export function routeForPolicy(policy='specialist_executor'){
  ...
  // Cheap/routine work prefers existing TensorMux credits, but remains available if that provider is down.
  if(!providerConfigured(provider))provider=provider==='tensormux'&&providerConfigured('agentrouter')?'agentrouter':provider==='agentrouter'&&providerConfigured('tensormux')?'tensormux':provider;
  ...
  return {policy:key,provider,model};
}
...
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null}={}){
  ...
  if(!cfg.base||!cfg.key)throw Object.assign(Error(`${chosenProvider}_not_configured`),{status:503,provider:chosenProvider,policy});
