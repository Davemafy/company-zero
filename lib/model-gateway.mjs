const POLICIES={
  ...
  frontier_escalation:{provider:'gemini',modelEnv:'GEMINI_FRONTIER_MODEL',fallbackModel:'gemini-3.8-flash',strictProvider:true},
...
export async function completeJson({system,user,policy='specialist_executor',role=null,timeoutMs=null,temperature=0,model=null,provider=null,onTelemetry=null}={}){
  ...
  const releaseLane=await acquireProviderLane(chosenProvider),startedAt=now(),controller=new AbortController(),limit=timeoutMs==null?cfg.timeout:Math.min(180000,Math.max(1000,Number(timeoutMs)||cfg.timeout)),timer=setTimeout(()=>controller.abort(),Math.min(180000,Math.max(1000,limit)));let body=null;
  try{
    ...
    const call=telemetry({provider:chosenProvider,policy,role,model:chosenModel,startedAt,body});try{await onTelemetry?.(call)}catch{}return {json,usage:call.usage,id:call.requestId,model:call.model,provider:chosenProvider,policy,role:call.role,latencyMs:call.latencyMs,costUsd:call.costUsd,telemetry:call};
