...
const LOCATION_STOPWORDS=new Set(['market','markets','business','businesses','startup','startups','entrepreneur','entrepreneurs','people','customers','clients','companies','company','ideas','opportunities','research','small','large','local','global','online']);
function localizedOpportunityLocation(text){
  const lower=String(text||'').toLowerCase();
  if(!/\b(ideas?|opportunit(?:y|ies)|recommendations?)\b/.test(lower)||!/\b(entrepreneurship|entrepreneur|business|startup|venture|market)\b/.test(lower))return null;
  const match=lower.match(/\b(?:in|for)\s+([a-z][a-z-]{2,})\b/),value=match?.[1]||'';
  return value&&!LOCATION_STOPWORDS.has(value)?value:null;
}
function queryVariants(request){
  ...
  const location=localizedOpportunityLocation(text);
  if(location)variants=[text,`${location} SME small business sectors official statistics`,`${location} entrepreneurship startup market opportunities`];
...
const DOMAIN_GROUPS=[
  ...
  {match:/\b(packaged food|food brand|snack|snacks|cpg|consumer packaged|grocery)\b/i,terms:['snack','snacks','packaged','cpg','grocery','retail','brand','brands','consumer']},
  {match:/\b(restaurant|restaurants|dining|cafe|café)\b/i,terms:['restaurant','restaurants','dining','cafe','café']},
...
function resultTokens(result){return new Set(`${result.title||''} ${result.snippet||''} ${result.url||''}`.toLowerCase().match(/[a-z0-9]+/g)||[])}
function satisfiesRequiredContext(result,request){
  const location=localizedOpportunityLocation(request);if(!location)return true;
  return resultTokens(result).has(location);
}
function relevanceScore(result,request){
  const anchors=relevanceAnchors(request);
  if(!anchors.length)return 1;
  const tokens=resultTokens(result);
  return anchors.reduce((score,anchor)=>score+(tokens.has(anchor)?1:0),0);
}
export function isRelevantSearchResult(result,request){return relevanceScore(result,request)>0&&satisfiesRequiredContext(result,request)}
...
export async function searchPublicWeb(request,{limit=MAX_RESULTS,timeoutMs=DEFAULT_TIMEOUT_MS,fetchImpl=fetch}={}){
  ...
  merged.sort((a,b)=>b.score-a.score);const selected=merged.filter(x=>x.score>=0&&x.relevanceScore>0&&satisfiesRequiredContext(x,request)).slice(0,limit);
