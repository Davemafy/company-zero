...
function classify(text,interactionRoute=''){
  ...
  const scores={general:0,lead_generation:0,company_creation:0,research:0,product_build:0,artifact_creation:0,optimization:0};
  ...
  add('product_build',/\b(site|website|app|api|software|code|frontend|backend|product)\b[^.]{0,50}\b(build|make|create|fix|improve|ship|design)\b|\b(build|make|create|fix|improve|ship|design)\b[^.]{0,50}\b(site|website|app|api|software|code|frontend|backend)\b/,6);
  ...
  // Customer language is common inside venture-building prompts. It must never, by itself, turn the organization into a prospecting team.
...
export function qualityCheck(result,plan){
  const files=Array.isArray(result?.files)?result.files:[];const text=files.map(f=>clean(f?.content)).join('\n').toLowerCase();const reasons=[];
  if(!files.length)reasons.push('no_files');if(text.length<220)reasons.push('too_thin');if(/could not complete|try again|reasoning service|request was saved/.test(text))reasons.push('workflow_theatre');
  ...
  const missing=(plan.contract?.required||[]).filter(k=>checks[k]&&!checks[k].test(text));if(missing.length)reasons.push(`contract_mismatch:${missing.join('|')}`);
  return {ok:reasons.length===0,reasons,contract:{required:plan.contract?.required||[],missing},structuralFailure:missing.length>=2||reasons.includes('missing_company_substance')};
}
