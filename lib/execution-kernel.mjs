...
function classify(text,interactionRoute=''){
  const lower=text.toLowerCase().replace(/\benterpr(?:enuer|eneur)\s*ship\b/g,'entrepreneurship').replace(/\benterprenuer\b/g,'entrepreneur').replace(/\bnigerai\b/g,'nigeria');
  const scores={general:0,external_action:0,lead_generation:0,company_creation:0,sales_strategy:0,research:0,product_build:0,artifact_creation:0,optimization:0};
  ...
  add('research',/\b(?:entrepreneurship|entrepreneur|business|startup|venture)\b[^.]{0,80}\b(?:ideas?|opportunit(?:y|ies)|recommendations?)\b[^.]{0,80}\b(?:in|for)\b|\b(?:ideas?|opportunit(?:y|ies)|recommendations?)\b[^.]{0,80}\b(?:for|in)\b[^.]{0,50}\b(?:entrepreneurship|entrepreneur|business|startup|venture|market)\b/,8);
  ...
  add('external_action',/\b(change|update|move|reschedule|schedule|send|post|publish|delete|remove|cancel|book|reserve|deploy|merge|approve|invite|add|transfer|pay|purchase|order)\b[^.]{0,80}\b(calendar|event|meeting|email|message|slack|github|pull request|\bpr\b|deployment|booking|reservation|account|settings|profile|subscription|invoice|payment|bank|drive|document|sheet)\b|\b(calendar|event|meeting|email|message|slack|github|pull request|\bpr\b|deployment|booking|reservation|account|settings|profile|subscription|invoice|payment|bank|drive|document|sheet)\b[^.]{0,80}\b(change|update|move|reschedule|schedule|send|post|publish|delete|remove|cancel|book|reserve|deploy|merge|approve|invite|add|transfer|pay|purchase|order)\b/,10);
...
function requestedContract(text,kind){
  ...
  return {kind,required:[...new Set(required)],askPolicy:/don'?t wait|do not wait|unless.*blocked/.test(lower)?'continue_unless_blocked':'ask_if_materially_blocked',externalActionRequired:kind==='external_action',requestedOutcomeModality:kind==='external_action'?'external_system':'digital',completionEvidenceRequired:kind==='external_action'};
}
...
export function compileExecutionPlan(request,{interactionRoute='',needsFreshEvidence=false}={}){
  ...
  const templates={
    external_action:{objective:'Cause the requested external-system change when an authorized actuator exists; otherwise preserve useful preparation without claiming completion.',units:['Resolve the target system and requested change','Check authority and actuator availability','Execute the external change only through an authorized capability','Capture an execution receipt and verify resulting state'],outputs:['action-status.md'],evidence:false},
  ...
  const functions=kind==='company_creation'?['venture_strategy','brand','finance','go_to_market','verification']:kind==='lead_generation'?['prospect_research','qualification','outreach','verification']:kind==='sales_strategy'?['market_strategy','offer_design','channel_strategy','economics','verification']:kind==='external_action'?['execution','verification']:['planning','execution','verification'];
  return {kind,request:text,objective:spec.objective,workUnits:spec.units.map((title,index)=>({id:index+1,title,state:index===0?'active':'queued'})),expectedOutputs:spec.outputs,requiresFreshEvidence:Boolean(needsFreshEvidence||spec.evidence),contract,organization:{mission:kind,functions},assumptionsPolicy:'Infer low-risk reversible details; ask only when a missing fact would materially change the result or an irreversible action is required.',truthPolicy:'Do not claim external actions, measurements, customers, purchases, deployments, research, or verification without evidence. A generated plan, guide, draft, or artifact never proves that a requested external-system action occurred. When no external evidence is available, clearly label market, customer, feature, pricing, margin and channel specifics as proposals, assumptions or hypotheses.'};
}
...
export function qualityCheck(result,plan){
  ...
  const roleSummary=result?.roleTelemetrySummary||null;
  if(Number(roleSummary?.expectedRoles)>0&&Number(roleSummary?.completedRoles)<Number(roleSummary?.expectedRoles))reasons.push(`organization_incomplete:${Number(roleSummary.completedRoles||0)}/${Number(roleSummary.expectedRoles||0)}`);
  if(plan.contract?.externalActionRequired){
    const evidence=result?.externalActionEvidence||result?.actionEvidence||result?.executionEvidence||null;
    const confirmed=Boolean(Array.isArray(evidence)?evidence.some(x=>x?.confirmed===true||['completed','succeeded','verified'].includes(String(x?.status||'').toLowerCase())):evidence&&(evidence.confirmed===true||['completed','succeeded','verified'].includes(String(evidence?.status||'').toLowerCase())));
    if(!confirmed)reasons.push('external_action_unverified');
  }
  ...
  if(plan.kind==='research'){
    if(!/(https?:\/\/|source|citation|evidence)/.test(text)||!/(compare|finding|conclusion|recommend|idea|opportunit|evaluation)/.test(text))reasons.push('missing_research_substance');
  }
  ...
  const structuralReasons=['external_action_unverified','missing_company_substance','missing_sales_substance','missing_lead_substance','missing_research_substance','missing_build_substance','missing_optimization_substance','missing_artifact_substance','unsupported_specific_claims'];
  return {ok:reasons.length===0,reasons,contract:{required:plan.contract?.required||[],missing},structuralFailure:missing.length>=2||structuralReasons.some(reason=>reasons.includes(reason))||reasons.some(reason=>reason.startsWith('organization_incomplete:'))};
}
