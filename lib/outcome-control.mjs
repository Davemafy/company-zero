...
export async function transitionDeliverable(companyId,deliverableId,next,{evidenceIds=[],operationId=null,reason=null}={}){
  const d=await get(deliverableId);
  if(!d||d.company_id!==companyId||d.kind!=='deliverable')throw new DomainError('deliverable_not_found',404);
  if(!LIFECYCLE.includes(next))throw new DomainError('invalid_deliverable_state',422,{next});
  const mergedEvidence=[...new Set([...(d.data.evidenceIds||[]),...clean(evidenceIds)])];
  const mergedOperations=operationId?[...new Set([...(d.data.operationIds||[]),operationId])]:(d.data.operationIds||[]);
  if(d.state===next){
    const evidenceChanged=mergedEvidence.length!==(d.data.evidenceIds||[]).length;
    const operationsChanged=mergedOperations.length!==(d.data.operationIds||[]).length;
    const reasonChanged=Boolean(reason&&d.data.lastReason!==reason);
    if(!evidenceChanged&&!operationsChanged&&!reasonChanged)return d;
    d.data.evidenceIds=mergedEvidence;
    d.data.operationIds=mergedOperations;
    if(reason)d.data.lastReason=reason;
    Object.assign(d,await put(d,{expectedVersion:d.version}));
    return d;
  }
  if(!TRANSITIONS[d.state]?.has(next))throw new DomainError('invalid_deliverable_transition',409,{from:d.state,to:next});
  d.state=next;
  d.data.evidenceIds=mergedEvidence;
  d.data.operationIds=mergedOperations;
  d.data.attempts=Number(d.data.attempts||0)+(next==='executing'||next==='producing'?1:0);
  if(reason)d.data.lastReason=reason;
  Object.assign(d,await put(d,{expectedVersion:d.version}));
  await emitRuntimeEvent(companyId,{sessionId:d.data.sessionId,missionId:d.data.missionId,type:`DELIVERABLE_${next.toUpperCase()}`,subjectKind:'deliverable',subjectId:d.id,data:{title:d.data.title,kind:d.data.kind},evidenceIds});
  if(['produced','executed','verified','observed','evaluated'].includes(next))await refreshDeliverableReadiness(companyId,d.data.graphId);
  return d
}
